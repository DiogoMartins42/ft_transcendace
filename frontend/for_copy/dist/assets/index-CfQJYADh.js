...
</section>`,se=`<div id="login-modal" class="fixed inset-0 flex flex-col items-end justify-start mt-22 mr-4 h-screen hidden">
...
`,re=`<!-- From Uiverse.io by themrsami --> 
...
`;function Y(){try{return JSON.parse(localStorage.getItem("userSession")||"null")}catch{return null}}function G(e,n){localStorage.setItem("userSession",JSON.stringify({token:e,user:n}))}function I(){localStorage.removeItem("userSession")}async function ce(){const e=Y();if(!e?.token){console.log("No stored session found");return}console.log("Found stored token, verifying...");try{const n=await fetch("undefined/auth/me",{headers:{Authorization:`Bearer ${e.token}`}});if(console.log("Verify response status:",n.status),!n.ok){console.warn("Stored session invalid, clearing..."),I();return}const o=n.headers.get("content-type");if(!o||!o.includes("application/json")){console.error("Response is not JSON:",o);const s=await n.text();console.error("Response body:",s),I();return}const t=await n.json();console.log("User data received:",t),T({isLoggedIn:!0,username:t.username,avatarUrl:t.avatarUrl||"/default-avatar.png"}),console.log("✅ Session restored:",t.username)}catch(n){console.error("Error verifying stored session:",n),I()}}const V=`<div class="flex flex-row gap-2 justify-center items-center">
    ...
    <div class="w-2 h-2 rounded-full bg-black animate-bounce [animation-delay:-.5s]"></div>
</div>`,ue="undefined/auth/login";function pe(){const e=document.getElementById("login-modal"),n=document.getElementById("signup-modal"),o=document.getElementById("open-login"),t=document.getElementById("open-signup"),s=document.getElementById("login-form"),r=document.getElementById("login-email"),i=document.getElementById("login-password"),l=document.getElementById("login-submit"),m=document.getElementById("login-message");if(!e||!o||!s||!r||!i||!l){console.warn("Login modal setup failed: missing elements.");return}function v(){const p=r.value.trim()&&i.value.trim();l.disabled=!p}r.addEventListener("input",v),i.addEventListener("input",v),v(),o.addEventListener("click",()=>e.classList.toggle("hidden")),e.addEventListener("click",p=>{p.target===e&&e.classList.add("hidden")}),t&&n&&t.addEventListener("click",()=>{e.classList.add("hidden"),n.classList.remove("hidden")}),s.addEventListener("submit",async p=>{if(p.preventDefault(),!r.value||!i.value)return;m&&(m.textContent="");const w=l.innerHTML;l.innerHTML=V,l.disabled=!0;try{const d=await fetch(ue,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:r.value,password:i.value})});if(!d.ok){m&&(m.textContent="Invalid email or password"),l.innerHTML=w,l.disabled=!1;return}const u=await d.json();u.token&&u.user&&(G(u.token,{username:u.user.username,avatarUrl:"/default-avatar.png"}),T({isLoggedIn:!0,username:u.user.username,avatarUrl:"/default-avatar.png"})),e.classList.add("hidden"),r.value="",i.value="",v()}catch(d){console.error("Login error:",d),m&&(m.textContent="An error occurred. Please try again.")}finally{l.innerHTML=w,v()}})}const me="undefined/auth/register";function he(){const e=document.getElementById("signup-modal"),n=document.getElementById("open-login"),o=document.getElementById("signup-form"),t=document.getElementById("signup-username"),s=document.getElementById("signup-email"),r=document.getElementById("signup-comfirmEmail"),i=document.getElementById("signup-password"),l=document.getElementById("signup-confirmPassword"),m=o?.querySelector('button[type="submit"]');if(!e||!o||!t||!s||!r||!i||!l||!m){console.warn("Signup modal setup failed: missing elements.");return}function v(d){return d.length>=8&&/[A-Z]/.test(d)&&/[0-9]/.test(d)}function p(d,u,b,g){u?(d.classList.remove("border-red-500","focus:ring-red-500"),d.classList.add("border-green-500","focus:ring-green-500")):(d.classList.remove("border-green-500","focus:ring-green-500"),d.classList.add("border-red-500","focus:ring-red-500"))}function w(){let d=!0;t.value.trim()?p(t,!0):(d=!1,p(t,!1)),s.value.trim()?p(s,!0):(d=!1,p(s,!1)),r.value.trim()!==s.value.trim()?(d=!1,p(r,!1)):p(r,!0),v(i.value)?p(i,!0):(d=!1,p(i,!1)),l.value!==i.value?(d=!1,p(l,!1)):p(l,!0),m.disabled=!d}[t,s,r,i,l].forEach(d=>d?.addEventListener("input",w)),n&&n.addEventListener("click",()=>e.classList.add("hidden")),e.addEventListener("click",d=>{d.target===e&&e.classList.add("hidden")}),o.addEventListener("submit",async d=>{d.preventDefault();const u=t.value.trim(),b=s.value.trim(),g=i.value.trim(),x=m.innerHTML;m.innerHTML=V,m.disabled=!0;try{const c=new AbortController,K=setTimeout(()=>c.abort(),15e3),H=await fetch(me,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:u,email:b,password:g}),signal:c.signal});if(clearTimeout(K),!H.ok){alert("Signup failed. Please try again."),i.value="",l.value="";return}const S=await H.json();S.token&&G(S.token,{username:S.username||u,avatarUrl:S.avatarUrl||"/default-avatar.png"}),T({isLoggedIn:!0,username:S.username||u,avatarUrl:S.avatarUrl||"/default-avatar.png"}),e.classList.add("hidden"),console.log("Signup successful!")}catch(c){console.error("Signup error:",c),alert("Network error or request timeout.")}finally{m.innerHTML=x,m.disabled=!1}})}async function ge(){const e=document.getElementById("user-section");if(!e){console.warn("⚠️ user-section not found in DOM");return}function n(){if(E.isLoggedIn){e.innerHTML=de;const o=document.getElementById("user-greeting");o&&(o.textContent=E.username||"User");const t=document.getElementById("user-avatar"),s=document.getElementById("default-avatar");E.avatarUrl?(t&&(t.src=E.avatarUrl,t.classList.remove("hidden")),s&&s.classList.add("hidden")):(t&&t.classList.add("hidden"),s&&s.classList.remove("hidden"));const r=document.getElementById("logout-btn");r&&r.addEventListener("click",()=>{I(),E.setState({isLoggedIn:!1,username:void 0,avatarUrl:void 0})})}else e.innerHTML=le,pe(),he()}n(),E.subscribe(n)}function fe(){const e=document.getElementById("menu-toggle"),n=document.getElementById("sidebar"),o=document.getElementById("page-content");!e||!n||!o||e.addEventListener("change",()=>{D.sidebarOpen=e.checked,D.sidebarOpen?(n.classList.remove("-translate-x-full"),o.classList.add("ml-48")):(n.classList.add("-translate-x-full"),o.classList.remove("ml-48"))})}let y=null;function W(e){const n=`wss://${window.location.hostname}:3000/ws`;return(!y||y.readyState===WebSocket.CLOSED||y.readyState===WebSocket.CLOSING)&&(y=new WebSocket(n),y.onopen=()=>{console.log("Connected to WebSocket:",n)},y.onclose=()=>{console.log("Disconnected from server!"),y=null},y.onerror=o=>{console.log("WebSocket error!",o),y=null}),y.onmessage=o=>{if(e)try{e(JSON.parse(o.data))}catch{console.warn("Received non-JSON message:",o.data)}},y}function ve(e){y&&y.readyState===WebSocket.OPEN?y.send(JSON.stringify(e)):console.warn("WebSocket not connected!")}function be(){const e=Y();return e?.user?.username?e.user.username:(window.anonymousId||(window.anonymousId="Anonymous"+Math.floor(1e3+Math.random()*9e3)),window.anonymousId)}function ye(){const e=document.querySelector("#chat-input"),n=document.querySelector("#chat-send"),o=document.getElementById("chat-messages");if(o&&(o.innerHTML=""),W(t=>{if(o)if(console.log("Received message:",t),t.type==="chat"){const s=document.createElement("div");s.className="mb-2 p-2 rounded bg-gray-100";const r=t.username||"Anonymous",i=t.text||"";s.textContent=`${r}: ${i}`,o.appendChild(s),o.scrollTop=o.scrollHeight}else if(t.type==="welcome"){const s=document.createElement("div");s.className="mb-2 p-2 rounded bg-blue-100 text-blue-800",s.textContent=t.message,o.appendChild(s),o.scrollTop=o.scrollHeight}else console.warn("Unknown message type:",t)}),e&&n){const t=()=>{e.value.trim()&&(ve({type:"chat",text:e.value.trim(),username:be()}),e.value="")};n.addEventListener("click",t),e.addEventListener("keypress",s=>{s.key==="Enter"&&t()})}}let a={difficulty:.05,mouse:!0,multiplayer:!1,resetSpeed:!0,paddleSpeed:5,ballSpeed:3,scoreLimit:5,bgColor:"#1C39BB",itemsColor:"#F5CB5C",default:!1};function xe(){const e=document.getElementById("openSettings"),n=document.getElementById("closeSettings"),o=document.getElementById("controlPanel"),t=document.getElementById("paddleSpeed"),s=document.getElementById("paddleSpeedValue"),r=document.getElementById("ballSpeed"),i=document.getElementById("ballSpeedValue"),l=document.getElementById("scoreLimit"),m=document.getElementById("bgColor"),v=document.getElementById("itemsColor"),p=document.getElementById("resetSpeed"),w=document.getElementById("resetDefaults"),d=document.querySelectorAll('input[name="difficulty"]'),u=document.querySelectorAll('input[name="mouse"]'),b=document.querySelectorAll('input[name="multiplayer"]');function g(){d.forEach(c=>{c.checked=Number(c.value)===a.difficulty}),u.forEach(c=>{c.checked=c.value==="true"===a.mouse}),b.forEach(c=>{c.checked=c.value==="true"===a.multiplayer}),p.checked=a.resetSpeed,t.value=a.paddleSpeed.toString(),s.textContent=t.value,r.value=a.ballSpeed.toString(),i.textContent=r.value,l.value=a.scoreLimit.toString(),m.value=a.bgColor,v.value=a.itemsColor}function x(){d.forEach(c=>{c.addEventListener("change",()=>{a.difficulty=Number(c.value)})}),u.forEach(c=>{c.addEventListener("change",()=>{a.multiplayer===!1&&(a.mouse=c.value==="true")})}),b.forEach(c=>{c.addEventListener("change",()=>{a.multiplayer=c.value==="true",a.mouse=!1})}),p.addEventListener("change",()=>{a.resetSpeed=p.checked}),t.addEventListener("input",()=>{a.paddleSpeed=Number(t.value),s.textContent=t.value}),r.addEventListener("input",()=>{a.ballSpeed=Number(r.value),i.textContent=r.value}),l.addEventListener("input",()=>{const c=Number(l.value);c>0&&(a.scoreLimit=c)}),m.addEventListener("input",()=>{a.bgColor=m.value}),v.addEventListener("input",()=>{a.itemsColor=v.value}),w.addEventListener("click",()=>{a={...a,difficulty:.05,mouse:!1,resetSpeed:!0,paddleSpeed:5,ballSpeed:3,scoreLimit:5,bgColor:"#1C39BB",itemsColor:"#F5CB5C",default:!0},g()})}e?.addEventListener("click",()=>{g(),o?.classList.toggle("hidden")}),n?.addEventListener("click",()=>{o?.classList.add("hidden")}),x()}function we(){if(!document.getElementById("stats-container"))return;const n=document.getElementById("fetchData"),o=document.getElementById("usernameInput"),t=document.getElementById("errorMessage"),s=document.getElementById("loading"),r=document.getElementById("infoUsername"),i=document.getElementById("infoEmail"),l=document.getElementById("winsCount"),m=document.getElementById("lossesCount"),v=document.getElementById("winRatio");async function p(u){s.style.display="block",t.style.display="none";try{const b=await fetch(`/stats/api/user-stats?username=${u}`),g=await fetch(`/stats/api/matches/${u}`);if(!b.ok||!g.ok)throw new Error("Invalid user");const x=await b.json(),c=await g.json();w(x,c)}catch(b){t.textContent=`Error: ${b.message}`,t.style.display="block"}finally{s.style.display="none"}}function w(u,b){r.textContent=u.username,i.textContent=u.email;let g=0,x=0;b.matches.forEach(c=>{c.winner===u.username?g++:c.loser===u.username&&x++}),l.textContent=String(g),m.textContent=String(x),v.textContent=`${(g/(g+x)*100||0).toFixed(1)}%`,d(b)}function d(u){const b=document.getElementById("matchHistoryGrid");if(b.innerHTML="",!u.matches||u.matches.length===0){b.innerHTML="<p>No matches yet</p>";return}u.matches.forEach(g=>{const x=document.createElement("div");x.classList.add("match-card"),x.innerHTML=`
        <h3>Match #${g.id}</h3>
        <div class="match-details">
      ...
      `,b.appendChild(x)})}n.addEventListener("click",()=>{const u=o.value.trim();u?p(u):(t.textContent="Please enter a username",t.style.display="block")})}async function Ce(e,n,o){var t,s,r,i;e>n?(r=e,i=n,t="nome",o?s="guest_multiplayer":s="bot"):(r=n,i=e,s="nome",o?t="guest_multiplayer":t="bot");try{const l=await fetch("/stats/api/match",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({winner:t,loser:s,winner_points:r,loser_points:i})});if(!l.ok){const v=await l.json();console.error("Failed to save match:",v);return}return await l.json()}catch(l){console.error("Error saving match:",l)}}let R=!1,O=!1;const h={START:"start",PLAYING:"playing",PAUSED:"paused",GAME_OVER:"gameOver"};let f=h.START,L=null,U=!1,_=!1,A=!1,N=!1;window.addEventListener("keydown",e=>{(e.key==="w"||e.key==="W")&&(U=!0),(e.key==="s"||e.key==="S")&&(_=!0),e.key==="ArrowUp"&&(A=!0),e.key==="ArrowDown"&&(N=!0)});window.addEventListener("keyup",e=>{(e.key==="w"||e.key==="W")&&(U=!1),(e.key==="s"||e.key==="S")&&(_=!1),e.key==="ArrowUp"&&(A=!1),e.key==="ArrowDown"&&(N=!1)});function Ee(e){const n=document.getElementById("game-overlay"),o=document.getElementById("game-message");!n||!o||(o.textContent=e,n.classList.remove("hidden"))}function B(e,n){const o=document.getElementById("game-overlay"),t=document.getElementById("overlay-buttons");!o||!t||(t.innerHTML="",e===1&&n.forEach(s=>{const r=document.createElement("button");r.textContent=s.text,r.className="px-8 py-2 text-white font-lucky text-lg rounded-full shadow-lg transition-transform 							transform bg-[#00091D] border-2 border-white hover:scale-105 hover:border-green-600 							hover:shadow-green-500/50 hover:shadow-2xl focus:outline-none hover:text-green-600",r.onclick=s.onClick,t.appendChild(r)}),e===2&&n.forEach(s=>{const r=document.createElement("button");r.textContent=s.text,r.className="w-35 px-8 py-2 text-white font-lucky text-lg rounded-lg shadow-lg transition-transform 							transform bg-[#00091D] border-2 border-white hover:scale-105 hover:border-blue-600 							hover:shadow-blue-500/50 hover:shadow-2xl focus:outline-none hover:text-blue-600",r.onclick=s.onClick,t.appendChild(r)}),o.classList.remove("hidden"))}function C(){const e=document.getElementById("game-overlay");e&&e.classList.add("hidden")}function Se(){_e();const{canvas:e,context:n}=Ie();if(!e||!n)return;const o={x:30,y:e.height/2-100/2,width:10,height:100,score:0},t={x:e.width-40,y:e.height/2-100/2,width:10,height:100,score:0},s={x:e.width/2,y:e.height/2,radius:10,velocityX:0,velocityY:0,speed:a.ballSpeed},r={x:e.width/2-1,y:0,width:2,height:10};o.y=e.height/2-o.height/2,t.y=e.height/2-t.height/2,s.x=e.width/2,s.y=e.height/2,s.velocityX=0,s.velocityY=0;const i=document.getElementById("pause-btn"),l=document.getElementById("openSettings"),m=w=>{ke(w,e,o)};a.mouse&&!a.multiplayer&&e.addEventListener("mousemove",m),B(1,[{text:"Start",onClick:()=>{f=h.PLAYING,C(),F(s)}}]),i&&i.addEventListener("click",()=>{f===h.PLAYING&&(f=h.PAUSED,B(2,[{text:"Resume",onClick:()=>{f=h.PLAYING,C()}},{text:"Restart",onClick:()=>{M(e,o,t,s)}}]))}),l&&l.addEventListener("click",()=>{f===h.PLAYING&&(f=h.PAUSED,B(2,[{text:"Resume",onClick:()=>{f=h.PLAYING,C()}},{text:"Restart",onClick:()=>{M(e,o,t,s)}}]))}),window.addEventListener("keydown",w=>{w.code==="Space"&&(f===h.PLAYING?(f=h.PAUSED,B(2,[{text:"Resume",onClick:()=>{f=h.PLAYING,C()}},{text:"Restart",onClick:()=>{M(e,o,t,s)}}])):f===h.PAUSED?(f=h.PLAYING,C()):f===h.START&&(f=h.PLAYING,C(),F(s)))});function v(){if(f===h.START||f===h.PAUSED||f===h.GAME_OVER){z(e,n,o,t,s,r);return}a.multiplayer&&Be(e,o,t),!a.multiplayer&&!a.mouse&&Le(e,o),Ue(e,o,t,s),z(e,n,o,t,s,r)}function p(){(!a.mouse||a.multiplayer)&&e.removeEventListener("mousemove",m),v(),L=requestAnimationFrame(p)}L=requestAnimationFrame(p)}function ke(e,n,o){const t=n.getBoundingClientRect();let s=e.clientY-t.top-o.height/2;s<0&&(s=0),s+o.height>n.height&&(s=n.height-o.height),o.y=s}function M(e,n,o,t){n.score=0,o.score=0,n.y=e.height/2-n.height/2,o.y=e.height/2-o.height/2,t.x=e.width/2,t.y=e.height/2,t.velocityX=0,t.velocityY=0,f=h.PLAYING,C(),F(t)}function F(e){let n=e.speed;R===!1&&(n=a.ballSpeed,R=!0);const o=Math.random()*Math.PI/2-Math.PI/4,t=Math.random()<.5?1:-1;e.velocityX=t*n*Math.cos(o),e.velocityY=n*Math.sin(o)}function Be(e,n,o){U&&(n.y-=a.paddleSpeed),_&&(n.y+=a.paddleSpeed),A&&(o.y-=a.paddleSpeed),N&&(o.y+=a.paddleSpeed),n.y=Math.max(0,Math.min(n.y,e.height-n.height)),o.y=Math.max(0,Math.min(o.y,e.height-o.height))}function Le(e,n){U&&(n.y-=a.paddleSpeed),_&&(n.y+=a.paddleSpeed),n.y=Math.max(0,Math.min(n.y,e.height-n.height))}function Ie(){const e=document.getElementById("pong");return e?{canvas:e,context:e.getContext("2d")}:{canvas:null,context:null}}function P(e,n,o,t,s,r,i=!1,l="none"){e.fillStyle=r,e.fillRect(n,o,t,s),i&&(e.strokeStyle=l,e.lineWidth=2,e.strokeRect(0,0,t,s))}function J(e,n,o,t,s){e.fillStyle=s,e.beginPath(),e.arc(n,o,t,0,Math.PI*2,!1),e.closePath(),e.fill()}function j(e,n,o,t,s){e.fillStyle=s,e.font="45px fantasy",e.fillText(n.toString(),o,t)}function Me(e,n,o){J(n,o.x,0,30,a.itemsColor);for(let t=0;t<=e.height;t+=15)P(n,o.x,o.y+t,o.width,o.height,a.itemsColor)}function z(e,n,o,t,s,r){P(n,0,0,e.width,e.height,a.bgColor,!0,a.itemsColor),Me(e,n,r),j(n,o.score,e.width/4,e.height/5,a.itemsColor),j(n,t.score,3*e.width/4,e.height/5,a.itemsColor),P(n,o.x,o.y,o.width,o.height,a.itemsColor),P(n,t.x,t.y,t.width,t.height,a.itemsColor),J(n,s.x,s.y,s.radius,a.itemsColor)}function Pe(e,n){return e.top=e.y-e.radius,e.bottom=e.y+e.radius,e.left=e.x-e.radius,e.right=e.x+e.radius,n.top=n.y,n.bottom=n.y+n.height,n.left=n.x,n.right=n.x+n.width,e.right>n.left&&e.bottom>n.top&&e.left<n.right&&e.top<n.bottom}function $(e,n){n.x=e.width/2,n.y=e.height/2,a.resetSpeed&&(n.speed=a.ballSpeed),F(n)}function Fe(e,n,o){const t=n.y+n.height/2;n.y+=(o.y-t)*a.difficulty,n.y=Math.max(0,Math.min(n.y,e.height-n.height))}function Ue(e,n,o,t){t.x+=t.velocityX,t.y+=t.velocityY,a.multiplayer||Fe(e,o,t),t.y+t.radius>e.height?(t.y=e.height-t.radius,t.velocityY=-t.velocityY):t.y-t.radius<0&&(t.y=t.radius,t.velocityY=-t.velocityY);const s=t.x<e.width/2?n:o;if(Pe(t,s)){O===!1&&(t.speed=a.ballSpeed,O=!0);let r=t.y-(s.y+s.height/2);r=r/(s.height/2);const i=r*Math.PI/4,l=t.x<e.width/2?1:-1;t.velocityX=l*t.speed*Math.cos(i),t.velocityY=t.speed*Math.sin(i),t.speed+=.1,l===1?t.x=s.x+s.width+t.radius:t.x=s.x-t.radius}if(t.x-t.radius<0?(o.score++,$(e,t)):t.x+t.radius>e.width&&(n.score++,$(e,t)),n.score===a.scoreLimit||o.score===a.scoreLimit){f=h.GAME_OVER;let r;n.score===a.scoreLimit?r="PLAYER 1 WINS!!!":a.multiplayer?r="PLAYER 2 WINS!!!":r="PLAYER 1 LOSES!!!",B(1,[{text:"Restart",onClick:()=>{M(e,n,o,t)}}]),Ee(r),setTimeout(()=>{Ce(n.score,o.score,a.multiplayer)},16)}}function _e(){L!==null&&(cancelAnimationFrame(L),L=null)}const De=document.querySelector("#app"),D={sidebarOpen:!1};class Ae{constructor(){this._isLoggedIn=!1,this.listeners=[]}get isLoggedIn(){return this._isLoggedIn}get username(){return this._username}get avatarUrl(){return this._avatarUrl}setState(n){let o=!1;n.isLoggedIn!==void 0&&n.isLoggedIn!==this._isLoggedIn&&(this._isLoggedIn=n.isLoggedIn,o=!0),n.username!==void 0&&n.username!==this._username&&(this._username=n.username,o=!0),n.avatarUrl!==void 0&&n.avatarUrl!==this._avatarUrl&&(this._avatarUrl=n.avatarUrl,o=!0),o&&this.listeners.forEach(t=>t())}subscribe(n){this.listeners.push(n)}}const E=new Ae;function T(e){E.setState(e)}async function k(e){De.innerHTML=`
    ${Z}
    ...
    ${ae}
    ${ie}
  `,ge(),fe(),W(n=>{const o=document.getElementById("chat-messages");if(!o)return;const t=document.createElement("div"),s=r=>{if(r==null)return"";if(typeof r=="string")try{const i=JSON.parse(r);return String(i&&typeof i=="object"?i.chat??i.message??JSON.stringify(i):i)}catch{return r}if(typeof r=="object"){const i=r;return String(i.chat??i.message??JSON.stringify(i))}return String(r)};t.textContent=s(n),o.appendChild(t),o.scrollTop=o.scrollHeight}),ye(),Se(),xe(),we()}async function q(){switch(window.location.hash.slice(1)||"home"){case"about":await k(Q);break;case"chat":await k(ee);break;case"contact":await k(ne);break;case"stats":await k(te);break;case"userSettings":await k(oe);break;default:await k(X)}D.sidebarOpen=!1}window.addEventListener("DOMContentLoaded",async()=>{await ce(),await q()});window.addEventListener("hashchange",q);
