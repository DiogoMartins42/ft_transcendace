...
`,$=`<div class="flex flex-row gap-2 justify-center items-center">
    ...
    <div class="w-2 h-2 rounded-full bg-black animate-bounce [animation-delay:-.5s]"></div>
</div>`,ue="http://localhost:3000/auth/login";function pe(){const e=document.getElementById("login-modal"),n=document.getElementById("signup-modal"),o=document.getElementById("open-login"),t=document.getElementById("open-signup"),s=document.getElementById("login-form"),r=document.getElementById("login-email"),i=document.getElementById("login-password"),l=document.getElementById("login-submit"),p=document.getElementById("login-message");if(!e||!o||!s||!r||!i||!l){console.warn("Login modal setup failed: missing elements.");return}o.addEventListener("click",()=>{e.classList.toggle("hidden")}),e.addEventListener("click",c=>{c.target===e&&e.classList.add("hidden")}),t&&n&&t.addEventListener("click",()=>{e.classList.add("hidden"),n.classList.remove("hidden")});function y(){r.value.trim()&&i.value.trim()?l.disabled=!1:l.disabled=!0}r.addEventListener("input",y),i.addEventListener("input",y);function x(c){p&&(p.textContent=c,p.classList.remove("hidden"),p.classList.toggle("text-red-500"))}function C(){p&&p.classList.add("hidden")}s.addEventListener("submit",async c=>{c.preventDefault(),C();const g=l.innerHTML;l.innerHTML=`${$}`,l.disabled=!0;const h=new AbortController,m=setTimeout(()=>h.abort(),15e3);try{const d=await fetch(ue,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:r.value,password:i.value}),signal:h.signal});if(clearTimeout(m),!d.ok){x("Invalid email or password"),i.value="",y(),l.innerHTML=g,l.disabled=!1;return}const u=await d.json();v.isLoggedIn=!0,v.username=u.username,v.avatarUrl=u.avatarUrl,e.classList.add("hidden"),r.value="",i.value="",l.innerHTML=g,l.disabled=!0}catch{h.signal.aborted?(x("Login request timed out. Please try again."),r.value="",i.value=""):x("An error occurred. Please try again."),l.innerHTML=g,l.disabled=!0}}),document.getElementById("forgotPass").addEventListener("click",()=>{v.isLoggedIn=!0,e.classList.add("hidden")})}const me="http://localhost:3000/auth/register";function he(){const e=document.getElementById("signup-modal"),n=document.getElementById("open-login"),o=document.getElementById("signup-form"),t=document.getElementById("signup-username"),s=document.getElementById("signup-email"),r=document.getElementById("signup-comfirmEmail"),i=document.getElementById("signup-password"),l=document.getElementById("signup-confirmPassword"),p=o?.querySelector('button[type="submit"]');if(!e||!o||!t||!s||!r||!i||!l||!p){console.warn("Signup modal setup failed: missing elements.");return}function y(u){let w=u.nextElementSibling;return(!w||!w.classList.contains("error-msg"))&&(w=document.createElement("span"),w.className="error-msg text-red-500 text-sm block hidden",u.insertAdjacentElement("afterend",w)),w}const x=y(t),C=y(s),B=y(r),c=y(i),g=y(l);p.disabled=!0;function h(u){const w=u.length>=8,S=/[A-Z]/.test(u),E=/[0-9]/.test(u);return w&&S&&E}function m(u,w,S,E){w?(u.classList.remove("border-red-500","focus:ring-red-500"),u.classList.add("border-green-500","focus:ring-green-500"),E&&E.classList.add("hidden")):(u.classList.remove("border-green-500","focus:ring-green-500"),u.classList.add("border-red-500","focus:ring-red-500"),E&&S&&(E.textContent=S,E.classList.remove("hidden")))}function d(){let u=!0;t.value.trim().length===0?(m(t,!1,"Username is required.",x),u=!1):m(t,!0,"",x),s.value.trim().length===0?(m(s,!1,"Email is required.",C),u=!1):m(s,!0,"",C),r.value.trim()!==s.value.trim()||r.value.trim()===""?(m(r,!1,"Emails do not match.",B),u=!1):m(r,!0,"",B),h(i.value)?m(i,!0,"",c):(m(i,!1,"Password must be ≥ 8 chars, 1 uppercase & 1 number.",c),u=!1),l.value!==i.value||l.value===""?(m(l,!1,"Passwords do not match.",g),u=!1):m(l,!0,"",g),p.disabled=!u}t.addEventListener("input",d),s.addEventListener("input",d),r.addEventListener("input",d),i.addEventListener("input",d),l.addEventListener("input",d),n&&n.addEventListener("click",()=>{e.classList.add("hidden")}),e.addEventListener("click",u=>{u.target===e&&e.classList.add("hidden")}),o.addEventListener("submit",async u=>{u.preventDefault();const w=t.value.trim(),S=s.value.trim(),E=i.value.trim(),K=p.innerHTML;p.innerHTML=$,p.disabled=!0;try{const P=new AbortController,X=setTimeout(()=>P.abort(),15e3),R=await fetch(me,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:w,email:S,password:E}),signal:P.signal});if(clearTimeout(X),R.ok){const J=await R.json();console.log("Signup response:",J),v.isLoggedIn=!0,v.username=w,v.avatarUrl="/default-avatar.png",e.classList.add("hidden"),console.log("Signup successful!")}else alert("Signup failed. Please try again."),i.value="",l.value=""}catch(P){console.error("Signup error:",P),alert("Network error or request timeout."),i.value="",l.value=""}finally{p.innerHTML=K,p.disabled=!1}})}async function ge(){const e=document.getElementById("user-section");if(!e){console.warn("⚠️ user-section not found in DOM");return}function n(){if(v.isLoggedIn){e.innerHTML=ce;const o=document.getElementById("user-greeting");o&&(o.textContent=v.username?` ${v.username}`:" User");const t=document.getElementById("user-avatar"),s=document.getElementById("default-avatar"),r=document.getElementById("user-avatar-modal"),i=document.getElementById("default-avatar-modal");v.avatarUrl?(t&&(t.src=v.avatarUrl,t.classList.remove("hidden")),s&&s.classList.add("hidden"),r&&(r.src=v.avatarUrl,r.classList.remove("hidden")),i&&i.classList.add("hidden")):(t&&t.classList.add("hidden"),s&&s.classList.remove("hidden"),r&&r.classList.add("hidden"),i&&i.classList.remove("hidden"));const l=document.getElementById("logout-btn");l&&l.addEventListener("click",()=>{v.isLoggedIn=!1,v.username=void 0,v.avatarUrl=void 0})}else e.innerHTML=de,pe(),he()}n(),v.subscribe&&v.subscribe(n)}function fe(){const e=document.getElementById("menu-toggle"),n=document.getElementById("sidebar"),o=document.getElementById("page-content");!e||!n||!o||e.addEventListener("change",()=>{T.sidebarOpen=e.checked,T.sidebarOpen?(n.classList.remove("-translate-x-full"),o.classList.add("ml-48")):(n.classList.add("-translate-x-full"),o.classList.remove("ml-48"))})}let k=null;function be(e){const n="ws://localhost:3000/ws";return k=new WebSocket(n),k.onopen=()=>{console.log("Connected to WebSocket:",n),V({type:"connection",text:"Hello from frontend!"})},k.onmessage=o=>{if(console.log("Message from server:",o.data),e)try{const t=JSON.parse(o.data);e(t),t.type==="welcome"?console.log("✅",t.message):t.type==="chat"&&console.log("💬",t.text)}catch{console.warn("Received non-json message:",o.data)}},k.onclose=()=>{console.log("Disconnected from server!")},k.onerror=o=>{console.log("WebSocket error!",o)},k}function V(e){k&&k.readyState===WebSocket.OPEN?k.send(JSON.stringify(e)):console.warn("WebSocket not connected!")}function ve(){const e=document.querySelector("#chat-input"),n=document.querySelector("#chat-send");e&&n&&n.addEventListener("click",()=>{e.value.trim()&&(V({type:"chat",text:e.value}),e.value="")})}let a={difficulty:.05,mouse:!0,multiplayer:!1,resetSpeed:!0,paddleSpeed:5,ballSpeed:3,scoreLimit:5,bgColor:"#1C39BB",itemsColor:"#F5CB5C",default:!1};function ye(){const e=document.getElementById("openSettings"),n=document.getElementById("closeSettings"),o=document.getElementById("controlPanel"),t=document.getElementById("paddleSpeed"),s=document.getElementById("paddleSpeedValue"),r=document.getElementById("ballSpeed"),i=document.getElementById("ballSpeedValue"),l=document.getElementById("scoreLimit"),p=document.getElementById("bgColor"),y=document.getElementById("itemsColor"),x=document.getElementById("resetSpeed"),C=document.getElementById("resetDefaults"),B=document.querySelectorAll('input[name="difficulty"]'),c=document.querySelectorAll('input[name="mouse"]'),g=document.querySelectorAll('input[name="multiplayer"]');function h(){B.forEach(d=>{d.checked=Number(d.value)===a.difficulty}),c.forEach(d=>{d.checked=d.value==="true"===a.mouse}),g.forEach(d=>{d.checked=d.value==="true"===a.multiplayer}),x.checked=a.resetSpeed,t.value=a.paddleSpeed.toString(),s.textContent=t.value,r.value=a.ballSpeed.toString(),i.textContent=r.value,l.value=a.scoreLimit.toString(),p.value=a.bgColor,y.value=a.itemsColor}function m(){B.forEach(d=>{d.addEventListener("change",()=>{a.difficulty=Number(d.value)})}),c.forEach(d=>{d.addEventListener("change",()=>{a.multiplayer===!1&&(a.mouse=d.value==="true")})}),g.forEach(d=>{d.addEventListener("change",()=>{a.multiplayer=d.value==="true",a.mouse=!1})}),x.addEventListener("change",()=>{a.resetSpeed=x.checked}),t.addEventListener("input",()=>{a.paddleSpeed=Number(t.value),s.textContent=t.value}),r.addEventListener("input",()=>{a.ballSpeed=Number(r.value),i.textContent=r.value}),l.addEventListener("input",()=>{const d=Number(l.value);d>0&&(a.scoreLimit=d)}),p.addEventListener("input",()=>{a.bgColor=p.value}),y.addEventListener("input",()=>{a.itemsColor=y.value}),C.addEventListener("click",()=>{a={...a,difficulty:.05,mouse:!1,resetSpeed:!0,paddleSpeed:5,ballSpeed:3,scoreLimit:5,bgColor:"#1C39BB",itemsColor:"#F5CB5C",default:!0},h()})}e?.addEventListener("click",()=>{h(),o?.classList.toggle("hidden")}),n?.addEventListener("click",()=>{o?.classList.add("hidden")}),m()}let O=!1,j=!1;const f={START:"start",PLAYING:"playing",PAUSED:"paused",GAME_OVER:"gameOver"};let b=f.START,A=!1,U=!1,H=!1,N=!1;window.addEventListener("keydown",e=>{(e.key==="w"||e.key==="W")&&(A=!0),(e.key==="s"||e.key==="S")&&(U=!0),e.key==="ArrowUp"&&(H=!0),e.key==="ArrowDown"&&(N=!0)});window.addEventListener("keyup",e=>{(e.key==="w"||e.key==="W")&&(A=!1),(e.key==="s"||e.key==="S")&&(U=!1),e.key==="ArrowUp"&&(H=!1),e.key==="ArrowDown"&&(N=!1)});function xe(e){const n=document.getElementById("game-overlay"),o=document.getElementById("game-message");!n||!o||(o.textContent=e,n.classList.remove("hidden"))}function M(e,n){const o=document.getElementById("game-overlay"),t=document.getElementById("overlay-buttons");!o||!t||(t.innerHTML="",e===1&&n.forEach(s=>{const r=document.createElement("button");r.textContent=s.text,r.className="px-8 py-2 text-white font-lucky text-lg rounded-full shadow-lg transition-transform 							transform bg-[#00091D] border-2 border-white hover:scale-105 hover:border-green-600 							hover:shadow-green-500/50 hover:shadow-2xl focus:outline-none hover:text-green-600",r.onclick=s.onClick,t.appendChild(r)}),e===2&&n.forEach(s=>{const r=document.createElement("button");r.textContent=s.text,r.className="w-35 px-8 py-2 text-white font-lucky text-lg rounded-lg shadow-lg transition-transform 							transform bg-[#00091D] border-2 border-white hover:scale-105 hover:border-blue-600 							hover:shadow-blue-500/50 hover:shadow-2xl focus:outline-none hover:text-blue-600",r.onclick=s.onClick,t.appendChild(r)}),o.classList.remove("hidden"))}function L(){const e=document.getElementById("game-overlay");e&&e.classList.add("hidden")}function we(){const{canvas:e,context:n}=Be();if(!e||!n)return;const o={x:30,y:e.height/2-100/2,width:10,height:100,score:0},t={x:e.width-40,y:e.height/2-100/2,width:10,height:100,score:0},s={x:e.width/2,y:e.height/2,radius:10,velocityX:0,velocityY:0,speed:a.ballSpeed},r={x:e.width/2-1,y:0,width:2,height:10};o.y=e.height/2-o.height/2,t.y=e.height/2-t.height/2,s.x=e.width/2,s.y=e.height/2,s.velocityX=0,s.velocityY=0;const i=document.getElementById("pause-btn"),l=document.getElementById("openSettings"),p=C=>{Ce(C,e,o)};a.mouse&&!a.multiplayer&&e.addEventListener("mousemove",p),M(1,[{text:"Start",onClick:()=>{b=f.PLAYING,L(),_(s)}}]),i&&i.addEventListener("click",()=>{b===f.PLAYING&&(b=f.PAUSED,M(2,[{text:"Resume",onClick:()=>{b=f.PLAYING,L()}},{text:"Restart",onClick:()=>{F(e,o,t,s)}}]))}),l&&l.addEventListener("click",()=>{b===f.PLAYING&&(b=f.PAUSED,M(2,[{text:"Resume",onClick:()=>{b=f.PLAYING,L()}},{text:"Restart",onClick:()=>{F(e,o,t,s)}}]))}),window.addEventListener("keydown",C=>{C.code==="Space"&&(b===f.PLAYING?(b=f.PAUSED,M(2,[{text:"Resume",onClick:()=>{b=f.PLAYING,L()}},{text:"Restart",onClick:()=>{F(e,o,t,s)}}])):b===f.PAUSED?(b=f.PLAYING,L()):b===f.START&&(b=f.PLAYING,L(),_(s)))});function y(){if(b===f.START||b===f.PAUSED||b===f.GAME_OVER){Y(e,n,o,t,s,r);return}a.multiplayer&&Ee(e,o,t),!a.multiplayer&&!a.mouse&&ke(e,o),Me(e,o,t,s),Y(e,n,o,t,s,r)}function x(){(!a.mouse||a.multiplayer)&&e.removeEventListener("mousemove",p),y(),requestAnimationFrame(x)}requestAnimationFrame(x)}function Ce(e,n,o){const t=n.getBoundingClientRect();let s=e.clientY-t.top-o.height/2;s<0&&(s=0),s+o.height>n.height&&(s=n.height-o.height),o.y=s}function F(e,n,o,t){n.score=0,o.score=0,n.y=e.height/2-n.height/2,o.y=e.height/2-o.height/2,t.x=e.width/2,t.y=e.height/2,t.velocityX=0,t.velocityY=0,b=f.PLAYING,L(),_(t)}function _(e){let n=e.speed;O===!1&&(n=a.ballSpeed,O=!0);const o=Math.random()*Math.PI/2-Math.PI/4,t=Math.random()<.5?1:-1;e.velocityX=t*n*Math.cos(o),e.velocityY=n*Math.sin(o)}function Ee(e,n,o){A&&(n.y-=a.paddleSpeed),U&&(n.y+=a.paddleSpeed),H&&(o.y-=a.paddleSpeed),N&&(o.y+=a.paddleSpeed),n.y=Math.max(0,Math.min(n.y,e.height-n.height)),o.y=Math.max(0,Math.min(o.y,e.height-o.height))}function ke(e,n){A&&(n.y-=a.paddleSpeed),U&&(n.y+=a.paddleSpeed),n.y=Math.max(0,Math.min(n.y,e.height-n.height))}function Be(){const e=document.getElementById("pong");return e?{canvas:e,context:e.getContext("2d")}:{canvas:null,context:null}}function D(e,n,o,t,s,r,i=!1,l="none"){e.fillStyle=r,e.fillRect(n,o,t,s),i&&(e.strokeStyle=l,e.lineWidth=2,e.strokeRect(0,0,t,s))}function W(e,n,o,t,s){e.fillStyle=s,e.beginPath(),e.arc(n,o,t,0,Math.PI*2,!1),e.closePath(),e.fill()}function z(e,n,o,t,s){e.fillStyle=s,e.font="45px fantasy",e.fillText(n.toString(),o,t)}function Le(e,n,o){W(n,o.x,0,30,a.itemsColor);for(let t=0;t<=e.height;t+=15)D(n,o.x,o.y+t,o.width,o.height,a.itemsColor)}function Y(e,n,o,t,s,r){D(n,0,0,e.width,e.height,a.bgColor,!0,a.itemsColor),Le(e,n,r),z(n,o.score,e.width/4,e.height/5,a.itemsColor),z(n,t.score,3*e.width/4,e.height/5,a.itemsColor),D(n,o.x,o.y,o.width,o.height,a.itemsColor),D(n,t.x,t.y,t.width,t.height,a.itemsColor),W(n,s.x,s.y,s.radius,a.itemsColor)}function Se(e,n){return e.top=e.y-e.radius,e.bottom=e.y+e.radius,e.left=e.x-e.radius,e.right=e.x+e.radius,n.top=n.y,n.bottom=n.y+n.height,n.left=n.x,n.right=n.x+n.width,e.right>n.left&&e.bottom>n.top&&e.left<n.right&&e.top<n.bottom}function G(e,n){n.x=e.width/2,n.y=e.height/2,a.resetSpeed&&(n.speed=a.ballSpeed),_(n)}function Ie(e,n,o){const t=n.y+n.height/2;n.y+=(o.y-t)*a.difficulty,n.y=Math.max(0,Math.min(n.y,e.height-n.height))}function Me(e,n,o,t){t.x+=t.velocityX,t.y+=t.velocityY,a.multiplayer||Ie(e,o,t),t.y+t.radius>e.height?(t.y=e.height-t.radius,t.velocityY=-t.velocityY):t.y-t.radius<0&&(t.y=t.radius,t.velocityY=-t.velocityY);const s=t.x<e.width/2?n:o;if(Se(t,s)){j===!1&&(t.speed=a.ballSpeed,j=!0);let r=t.y-(s.y+s.height/2);r=r/(s.height/2);const i=r*Math.PI/4,l=t.x<e.width/2?1:-1;t.velocityX=l*t.speed*Math.cos(i),t.velocityY=t.speed*Math.sin(i),t.speed+=.1,l===1?t.x=s.x+s.width+t.radius:t.x=s.x-t.radius}if(t.x-t.radius<0?(o.score++,G(e,t)):t.x+t.radius>e.width&&(n.score++,G(e,t)),n.score===a.scoreLimit||o.score===a.scoreLimit){b=f.GAME_OVER;let r;n.score===a.scoreLimit?r="PLAYER 1 WINS!!!":a.multiplayer?r="PLAYER 2 WINS!!!":r="PLAYER 1 LOSES!!!",M(1,[{text:"Restart",onClick:()=>{F(e,n,o,t)}}]),xe(r)}}function Pe(){if(!document.getElementById("stats-container"))return;console.log("Stats Script loaded");const n=document.getElementById("fetchData"),o=document.getElementById("usernameInput"),t=document.getElementById("errorMessage"),s=document.getElementById("loading"),r=document.getElementById("infoUsername"),i=document.getElementById("infoEmail"),l=document.getElementById("winsCount"),p=document.getElementById("lossesCount"),y=document.getElementById("winRatio");async function x(c){s.style.display="block",t.style.display="none";try{const g=await fetch(`/stats/api/user-stats?username=${c}`),h=await fetch(`/stats/api/matches/${c}`);if(!g.ok||!h.ok)throw new Error("Invalid user");const m=await g.json(),d=await h.json();C(m,d)}catch(g){t.textContent=`Error: ${g.message}`,t.style.display="block"}finally{s.style.display="none"}}function C(c,g){r.textContent=c.username,i.textContent=c.email;let h=0,m=0;g.matches.forEach(d=>{d.winner===c.username?h++:d.loser===c.username&&m++}),l.textContent=String(h),p.textContent=String(m),y.textContent=`${(h/(h+m)*100||0).toFixed(1)}%`,B(g)}function B(c){const g=document.getElementById("matchHistoryGrid");if(g.innerHTML="",!c.matches||c.matches.length===0){g.innerHTML="<p>No matches yet</p>";return}c.matches.forEach(h=>{const m=document.createElement("div");m.classList.add("match-card"),m.innerHTML=`
        <h3>Match #${h.id}</h3>
        <div class="match-details">
      ...
      `,g.appendChild(m)})}n.addEventListener("click",()=>{const c=o.value.trim();c?x(c):(t.textContent="Please enter a username",t.style.display="block")})}const Fe=document.querySelector("#app"),T={sidebarOpen:!1};class De{constructor(){this._isLoggedIn=!1,this.listeners=[]}get isLoggedIn(){return this._isLoggedIn}set isLoggedIn(n){this._isLoggedIn=n,this.listeners.forEach(o=>o())}subscribe(n){this.listeners.push(n)}}const v=new De;async function I(e){Fe.innerHTML=`
		${Q}
		...
		${ie}
		${le}
	`,ge(),fe(),be(n=>{const o=document.getElementById("chat-messages");if(!o)return;const t=document.createElement("div");t.textContent=n.chat||n.message||JSON.stringify(n),o.appendChild(t),o.scrollTop=o.scrollHeight}),ve(),we(),ye(),Pe()}function q(){switch(window.location.hash.slice(1)||"home"){case"about":I(ee);break;case"chat":I(te);break;case"contact":I(ne);break;case"stats":I(oe);break;case"userSettings":I(se);break;default:I(Z)}T.sidebarOpen=!1}window.addEventListener("DOMContentLoaded",q);window.addEventListener("hashchange",q);
