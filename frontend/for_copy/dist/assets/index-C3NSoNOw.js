(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const r of s)if(r.type==="childList")for(const i of r.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&n(i)}).observe(document,{childList:!0,subtree:!0});function o(s){const r={};return s.integrity&&(r.integrity=s.integrity),s.referrerPolicy&&(r.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?r.credentials="include":s.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function n(s){if(s.ep)return;s.ep=!0;const r=o(s);fetch(s.href,r)}})();const J=`<main id="page-content" class="pt-16 flex-1">
 ...
 `,K=`<nav class="bg-[#0E172B] text-[#242423] font-bold w-screen
...
</nav>`,X=`<section class="bg-amber-400 w-full h-screen m-0 p-0">
...
</section>`,ne=`<div id="login-modal" class="fixed inset-0 flex flex-col items-end justify-start mt-22 mr-4 h-screen hidden">
...
`,oe=`<!-- From Uiverse.io by themrsami --> 
...
`,ae=`<!-- <button id="open-login" class="px-4 py- border border-amber-400 rounded text-white hover:bg-amber-200 hover:text-gray-800 transition cursor-pointer">
    Log in
</button>
...
`;function le(){try{return JSON.parse(localStorage.getItem("userSession")||"null")}catch{return null}}function $(e,t){localStorage.setItem("userSession",JSON.stringify({token:e,user:t}))}function de(){localStorage.removeItem("userSession")}async function ce(){const e=le();if(e?.token)try{const t=await fetch("undefined/me",{headers:{Authorization:`Bearer ${e.token}`}});if(!t.ok){console.warn("Stored session invalid, clearing..."),de();return}const o=await t.json();T({isLoggedIn:!0,username:o.username,avatarUrl:"/default-avatar.png"}),console.log("✅ Session restored:",o.username)}catch(t){console.error("Error verifying stored session:",t)}}const Y=`<div class="flex flex-row gap-2 justify-center items-center">
    ...
    <div class="w-2 h-2 rounded-full bg-black animate-bounce [animation-delay:-.5s]"></div>
</div>`,ue="undefined/auth/login";function pe(){const e=document.getElementById("login-modal"),t=document.getElementById("signup-modal"),o=document.getElementById("open-login"),n=document.getElementById("open-signup"),s=document.getElementById("login-form"),r=document.getElementById("login-email"),i=document.getElementById("login-password"),l=document.getElementById("login-submit"),p=document.getElementById("login-message");if(!e||!o||!s||!r||!i||!l){console.warn("Login modal setup failed: missing elements.");return}o.addEventListener("click",()=>{e.classList.toggle("hidden")}),e.addEventListener("click",c=>{c.target===e&&e.classList.add("hidden")}),n&&t&&n.addEventListener("click",()=>{e.classList.add("hidden"),t.classList.remove("hidden")});function y(){l.disabled=!(r.value.trim()&&i.value.trim())}r.addEventListener("input",y),i.addEventListener("input",y);function m(c){p&&(p.textContent=c,p.classList.remove("hidden"),p.classList.add("text-red-500"))}function w(){p&&p.classList.add("hidden")}s.addEventListener("submit",async c=>{c.preventDefault(),w();const u=l.innerHTML;l.innerHTML=Y,l.disabled=!0;const h=new AbortController,g=setTimeout(()=>h.abort(),15e3);try{const b=await fetch(ue,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:r.value,password:i.value}),signal:h.signal});if(clearTimeout(g),!b.ok){m("Invalid email or password"),i.value="",y(),l.innerHTML=u,l.disabled=!1;return}const d=await b.json();T({isLoggedIn:!0,username:d.username,avatarUrl:d.avatarUrl}),d.token&&$(d.token,{username:d.username,avatarUrl:d.avatarUrl||"/default-avatar.png"}),e.classList.add("hidden"),r.value="",i.value="",l.innerHTML=u,l.disabled=!0}catch{h.signal.aborted?m("Login request timed out. Please try again."):m("An error occurred. Please try again."),l.innerHTML=u,l.disabled=!0}})}const me="undefined/auth/register";function he(){const e=document.getElementById("signup-modal"),t=document.getElementById("open-login"),o=document.getElementById("signup-form"),n=document.getElementById("signup-username"),s=document.getElementById("signup-email"),r=document.getElementById("signup-comfirmEmail"),i=document.getElementById("signup-password"),l=document.getElementById("signup-confirmPassword"),p=o?.querySelector('button[type="submit"]');if(!e||!o||!n||!s||!r||!i||!l||!p){console.warn("Signup modal setup failed: missing elements.");return}function y(c){return c.length>=8&&/[A-Z]/.test(c)&&/[0-9]/.test(c)}function m(c,u,h,g){u?(c.classList.remove("border-red-500","focus:ring-red-500"),c.classList.add("border-green-500","focus:ring-green-500")):(c.classList.remove("border-green-500","focus:ring-green-500"),c.classList.add("border-red-500","focus:ring-red-500"))}function w(){let c=!0;n.value.trim()?m(n,!0):(c=!1,m(n,!1)),s.value.trim()?m(s,!0):(c=!1,m(s,!1)),r.value.trim()!==s.value.trim()?(c=!1,m(r,!1)):m(r,!0),y(i.value)?m(i,!0):(c=!1,m(i,!1)),l.value!==i.value?(c=!1,m(l,!1)):m(l,!0),p.disabled=!c}[n,s,r,i,l].forEach(c=>c?.addEventListener("input",w)),t&&t.addEventListener("click",()=>e.classList.add("hidden")),e.addEventListener("click",c=>{c.target===e&&e.classList.add("hidden")}),o.addEventListener("submit",async c=>{c.preventDefault();const u=n.value.trim(),h=s.value.trim(),g=i.value.trim(),b=p.innerHTML;p.innerHTML=Y,p.disabled=!0;try{const d=new AbortController,q=setTimeout(()=>d.abort(),15e3),N=await fetch(me,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:u,email:h,password:g}),signal:d.signal});if(clearTimeout(q),!N.ok){alert("Signup failed. Please try again."),i.value="",l.value="";return}const S=await N.json();S.token&&$(S.token,{username:S.username||u,avatarUrl:S.avatarUrl||"/default-avatar.png"}),T({isLoggedIn:!0,username:S.username||u,avatarUrl:S.avatarUrl||"/default-avatar.png"}),e.classList.add("hidden"),console.log("Signup successful!")}catch(d){console.error("Signup error:",d),alert("Network error or request timeout.")}finally{p.innerHTML=b,p.disabled=!1}})}async function ge(){const e=document.getElementById("user-section");if(!e){console.warn("⚠️ user-section not found in DOM");return}function t(){if(E.isLoggedIn){e.innerHTML=ie;const o=document.getElementById("user-greeting");o&&(o.textContent=E.username||"User");const n=document.getElementById("user-avatar"),s=document.getElementById("default-avatar");E.avatarUrl?(n&&(n.src=E.avatarUrl,n.classList.remove("hidden")),s&&s.classList.add("hidden")):(n&&n.classList.add("hidden"),s&&s.classList.remove("hidden"));const r=document.getElementById("logout-btn");r&&r.addEventListener("click",()=>{E.setState({isLoggedIn:!1,username:void 0,avatarUrl:void 0})})}else e.innerHTML=ae,pe(),he()}t(),E.subscribe(t)}function fe(){const e=document.getElementById("menu-toggle"),t=document.getElementById("sidebar"),o=document.getElementById("page-content");!e||!t||!o||e.addEventListener("change",()=>{F.sidebarOpen=e.checked,F.sidebarOpen?(t.classList.remove("-translate-x-full"),o.classList.add("ml-48")):(t.classList.add("-translate-x-full"),o.classList.remove("ml-48"))})}let x=null;function G(e){const t=`ws://${window.location.hostname}:3000/ws`;return(!x||x.readyState===WebSocket.CLOSED||x.readyState===WebSocket.CLOSING)&&(x=new WebSocket(t),x.onopen=()=>{console.log("Connected to WebSocket:",t)},x.onclose=()=>{console.log("Disconnected from server!"),x=null},x.onerror=o=>{console.log("WebSocket error!",o),x=null}),x.onmessage=o=>{if(e)try{e(JSON.parse(o.data))}catch{console.warn("Received non-JSON message:",o.data)}},x}function ve(e){x&&x.readyState===WebSocket.OPEN?x.send(JSON.stringify(e)):console.warn("WebSocket not connected!")}function be(){const e=localStorage.getItem("username");return e||(window.anonymousId||(window.anonymousId="Anonymous"+Math.floor(1e3+Math.random()*9e3)),window.anonymousId)}function ye(){const e=document.querySelector("#chat-input"),t=document.querySelector("#chat-send"),o=document.getElementById("chat-messages");if(o&&(o.innerHTML=""),G(n=>{if(o)if(console.log("Received message:",n),n.type==="chat"){const s=document.createElement("div");s.className="mb-2 p-2 rounded bg-gray-100";const r=n.username||"Anonymous",i=n.text||"";s.textContent=`${r}: ${i}`,o.appendChild(s),o.scrollTop=o.scrollHeight}else if(n.type==="welcome"){const s=document.createElement("div");s.className="mb-2 p-2 rounded bg-blue-100 text-blue-800",s.textContent=n.message,o.appendChild(s),o.scrollTop=o.scrollHeight}else console.warn("Unknown message type:",n)}),e&&t){const n=()=>{e.value.trim()&&(ve({type:"chat",text:e.value.trim(),username:be()}),e.value="")};t.addEventListener("click",n),e.addEventListener("keypress",s=>{s.key==="Enter"&&n()})}}let a={difficulty:.05,mouse:!0,multiplayer:!1,resetSpeed:!0,paddleSpeed:5,ballSpeed:3,scoreLimit:5,bgColor:"#1C39BB",itemsColor:"#F5CB5C",default:!1};function xe(){const e=document.getElementById("openSettings"),t=document.getElementById("closeSettings"),o=document.getElementById("controlPanel"),n=document.getElementById("paddleSpeed"),s=document.getElementById("paddleSpeedValue"),r=document.getElementById("ballSpeed"),i=document.getElementById("ballSpeedValue"),l=document.getElementById("scoreLimit"),p=document.getElementById("bgColor"),y=document.getElementById("itemsColor"),m=document.getElementById("resetSpeed"),w=document.getElementById("resetDefaults"),c=document.querySelectorAll('input[name="difficulty"]'),u=document.querySelectorAll('input[name="mouse"]'),h=document.querySelectorAll('input[name="multiplayer"]');function g(){c.forEach(d=>{d.checked=Number(d.value)===a.difficulty}),u.forEach(d=>{d.checked=d.value==="true"===a.mouse}),h.forEach(d=>{d.checked=d.value==="true"===a.multiplayer}),m.checked=a.resetSpeed,n.value=a.paddleSpeed.toString(),s.textContent=n.value,r.value=a.ballSpeed.toString(),i.textContent=r.value,l.value=a.scoreLimit.toString(),p.value=a.bgColor,y.value=a.itemsColor}function b(){c.forEach(d=>{d.addEventListener("change",()=>{a.difficulty=Number(d.value)})}),u.forEach(d=>{d.addEventListener("change",()=>{a.multiplayer===!1&&(a.mouse=d.value==="true")})}),h.forEach(d=>{d.addEventListener("change",()=>{a.multiplayer=d.value==="true",a.mouse=!1})}),m.addEventListener("change",()=>{a.resetSpeed=m.checked}),n.addEventListener("input",()=>{a.paddleSpeed=Number(n.value),s.textContent=n.value}),r.addEventListener("input",()=>{a.ballSpeed=Number(r.value),i.textContent=r.value}),l.addEventListener("input",()=>{const d=Number(l.value);d>0&&(a.scoreLimit=d)}),p.addEventListener("input",()=>{a.bgColor=p.value}),y.addEventListener("input",()=>{a.itemsColor=y.value}),w.addEventListener("click",()=>{a={...a,difficulty:.05,mouse:!1,resetSpeed:!0,paddleSpeed:5,ballSpeed:3,scoreLimit:5,bgColor:"#1C39BB",itemsColor:"#F5CB5C",default:!0},g()})}e?.addEventListener("click",()=>{g(),o?.classList.toggle("hidden")}),t?.addEventListener("click",()=>{o?.classList.add("hidden")}),b()}function we(){if(!document.getElementById("stats-container"))return;const t=document.getElementById("fetchData"),o=document.getElementById("usernameInput"),n=document.getElementById("errorMessage"),s=document.getElementById("loading"),r=document.getElementById("infoUsername"),i=document.getElementById("infoEmail"),l=document.getElementById("winsCount"),p=document.getElementById("lossesCount"),y=document.getElementById("winRatio");async function m(u){s.style.display="block",n.style.display="none";try{const h=await fetch(`/stats/api/user-stats?username=${u}`),g=await fetch(`/stats/api/matches/${u}`);if(!h.ok||!g.ok)throw new Error("Invalid user");const b=await h.json(),d=await g.json();w(b,d)}catch(h){n.textContent=`Error: ${h.message}`,n.style.display="block"}finally{s.style.display="none"}}function w(u,h){r.textContent=u.username,i.textContent=u.email;let g=0,b=0;h.matches.forEach(d=>{d.winner===u.username?g++:d.loser===u.username&&b++}),l.textContent=String(g),p.textContent=String(b),y.textContent=`${(g/(g+b)*100||0).toFixed(1)}%`,c(h)}function c(u){const h=document.getElementById("matchHistoryGrid");if(h.innerHTML="",!u.matches||u.matches.length===0){h.innerHTML="<p>No matches yet</p>";return}u.matches.forEach(g=>{const b=document.createElement("div");b.classList.add("match-card"),b.innerHTML=`
        <h3>Match #${g.id}</h3>
        <div class="match-details">
      ...
      `,h.appendChild(b)})}t.addEventListener("click",()=>{const u=o.value.trim();u?m(u):(n.textContent="Please enter a username",n.style.display="block")})}async function Ce(e,t,o){var n,s,r,i;e>t?(r=e,i=t,n="nome",o?s="guest_multiplayer":s="bot"):(r=t,i=e,s="nome",o?n="guest_multiplayer":n="bot");try{const l=await fetch("/stats/api/match",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({winner:n,loser:s,winner_points:r,loser_points:i})});if(!l.ok){const y=await l.json();console.error("Failed to save match:",y);return}return await l.json()}catch(l){console.error("Error saving match:",l)}}let H=!1,R=!1;const f={START:"start",PLAYING:"playing",PAUSED:"paused",GAME_OVER:"gameOver"};let v=f.START,L=null,U=!1,_=!1,D=!1,A=!1;window.addEventListener("keydown",e=>{(e.key==="w"||e.key==="W")&&(U=!0),(e.key==="s"||e.key==="S")&&(_=!0),e.key==="ArrowUp"&&(D=!0),e.key==="ArrowDown"&&(A=!0)});window.addEventListener("keyup",e=>{(e.key==="w"||e.key==="W")&&(U=!1),(e.key==="s"||e.key==="S")&&(_=!1),e.key==="ArrowUp"&&(D=!1),e.key==="ArrowDown"&&(A=!1)});function Ee(e){const t=document.getElementById("game-overlay"),o=document.getElementById("game-message");!t||!o||(o.textContent=e,t.classList.remove("hidden"))}function B(e,t){const o=document.getElementById("game-overlay"),n=document.getElementById("overlay-buttons");!o||!n||(n.innerHTML="",e===1&&t.forEach(s=>{const r=document.createElement("button");r.textContent=s.text,r.className="px-8 py-2 text-white font-lucky text-lg rounded-full shadow-lg transition-transform 							transform bg-[#00091D] border-2 border-white hover:scale-105 hover:border-green-600 							hover:shadow-green-500/50 hover:shadow-2xl focus:outline-none hover:text-green-600",r.onclick=s.onClick,n.appendChild(r)}),e===2&&t.forEach(s=>{const r=document.createElement("button");r.textContent=s.text,r.className="w-35 px-8 py-2 text-white font-lucky text-lg rounded-lg shadow-lg transition-transform 							transform bg-[#00091D] border-2 border-white hover:scale-105 hover:border-blue-600 							hover:shadow-blue-500/50 hover:shadow-2xl focus:outline-none hover:text-blue-600",r.onclick=s.onClick,n.appendChild(r)}),o.classList.remove("hidden"))}function C(){const e=document.getElementById("game-overlay");e&&e.classList.add("hidden")}function Se(){Fe();const{canvas:e,context:t}=Ie();if(!e||!t)return;const o={x:30,y:e.height/2-100/2,width:10,height:100,score:0},n={x:e.width-40,y:e.height/2-100/2,width:10,height:100,score:0},s={x:e.width/2,y:e.height/2,radius:10,velocityX:0,velocityY:0,speed:a.ballSpeed},r={x:e.width/2-1,y:0,width:2,height:10};o.y=e.height/2-o.height/2,n.y=e.height/2-n.height/2,s.x=e.width/2,s.y=e.height/2,s.velocityX=0,s.velocityY=0;const i=document.getElementById("pause-btn"),l=document.getElementById("openSettings"),p=w=>{ke(w,e,o)};a.mouse&&!a.multiplayer&&e.addEventListener("mousemove",p),B(1,[{text:"Start",onClick:()=>{v=f.PLAYING,C(),P(s)}}]),i&&i.addEventListener("click",()=>{v===f.PLAYING&&(v=f.PAUSED,B(2,[{text:"Resume",onClick:()=>{v=f.PLAYING,C()}},{text:"Restart",onClick:()=>{I(e,o,n,s)}}]))}),l&&l.addEventListener("click",()=>{v===f.PLAYING&&(v=f.PAUSED,B(2,[{text:"Resume",onClick:()=>{v=f.PLAYING,C()}},{text:"Restart",onClick:()=>{I(e,o,n,s)}}]))}),window.addEventListener("keydown",w=>{w.code==="Space"&&(v===f.PLAYING?(v=f.PAUSED,B(2,[{text:"Resume",onClick:()=>{v=f.PLAYING,C()}},{text:"Restart",onClick:()=>{I(e,o,n,s)}}])):v===f.PAUSED?(v=f.PLAYING,C()):v===f.START&&(v=f.PLAYING,C(),P(s)))});function y(){if(v===f.START||v===f.PAUSED||v===f.GAME_OVER){j(e,t,o,n,s,r);return}a.multiplayer&&Be(e,o,n),!a.multiplayer&&!a.mouse&&Le(e,o),_e(e,o,n,s),j(e,t,o,n,s,r)}function m(){(!a.mouse||a.multiplayer)&&e.removeEventListener("mousemove",p),y(),L=requestAnimationFrame(m)}L=requestAnimationFrame(m)}function ke(e,t,o){const n=t.getBoundingClientRect();let s=e.clientY-n.top-o.height/2;s<0&&(s=0),s+o.height>t.height&&(s=t.height-o.height),o.y=s}function I(e,t,o,n){t.score=0,o.score=0,t.y=e.height/2-t.height/2,o.y=e.height/2-o.height/2,n.x=e.width/2,n.y=e.height/2,n.velocityX=0,n.velocityY=0,v=f.PLAYING,C(),P(n)}function P(e){let t=e.speed;H===!1&&(t=a.ballSpeed,H=!0);const o=Math.random()*Math.PI/2-Math.PI/4,n=Math.random()<.5?1:-1;e.velocityX=n*t*Math.cos(o),e.velocityY=t*Math.sin(o)}function Be(e,t,o){U&&(t.y-=a.paddleSpeed),_&&(t.y+=a.paddleSpeed),D&&(o.y-=a.paddleSpeed),A&&(o.y+=a.paddleSpeed),t.y=Math.max(0,Math.min(t.y,e.height-t.height)),o.y=Math.max(0,Math.min(o.y,e.height-o.height))}function Le(e,t){U&&(t.y-=a.paddleSpeed),_&&(t.y+=a.paddleSpeed),t.y=Math.max(0,Math.min(t.y,e.height-t.height))}function Ie(){const e=document.getElementById("pong");return e?{canvas:e,context:e.getContext("2d")}:{canvas:null,context:null}}function M(e,t,o,n,s,r,i=!1,l="none"){e.fillStyle=r,e.fillRect(t,o,n,s),i&&(e.strokeStyle=l,e.lineWidth=2,e.strokeRect(0,0,n,s))}function V(e,t,o,n,s){e.fillStyle=s,e.beginPath(),e.arc(t,o,n,0,Math.PI*2,!1),e.closePath(),e.fill()}function O(e,t,o,n,s){e.fillStyle=s,e.font="45px fantasy",e.fillText(t.toString(),o,n)}function Me(e,t,o){V(t,o.x,0,30,a.itemsColor);for(let n=0;n<=e.height;n+=15)M(t,o.x,o.y+n,o.width,o.height,a.itemsColor)}function j(e,t,o,n,s,r){M(t,0,0,e.width,e.height,a.bgColor,!0,a.itemsColor),Me(e,t,r),O(t,o.score,e.width/4,e.height/5,a.itemsColor),O(t,n.score,3*e.width/4,e.height/5,a.itemsColor),M(t,o.x,o.y,o.width,o.height,a.itemsColor),M(t,n.x,n.y,n.width,n.height,a.itemsColor),V(t,s.x,s.y,s.radius,a.itemsColor)}function Pe(e,t){return e.top=e.y-e.radius,e.bottom=e.y+e.radius,e.left=e.x-e.radius,e.right=e.x+e.radius,t.top=t.y,t.bottom=t.y+t.height,t.left=t.x,t.right=t.x+t.width,e.right>t.left&&e.bottom>t.top&&e.left<t.right&&e.top<t.bottom}function z(e,t){t.x=e.width/2,t.y=e.height/2,a.resetSpeed&&(t.speed=a.ballSpeed),P(t)}function Ue(e,t,o){const n=t.y+t.height/2;t.y+=(o.y-n)*a.difficulty,t.y=Math.max(0,Math.min(t.y,e.height-t.height))}function _e(e,t,o,n){n.x+=n.velocityX,n.y+=n.velocityY,a.multiplayer||Ue(e,o,n),n.y+n.radius>e.height?(n.y=e.height-n.radius,n.velocityY=-n.velocityY):n.y-n.radius<0&&(n.y=n.radius,n.velocityY=-n.velocityY);const s=n.x<e.width/2?t:o;if(Pe(n,s)){R===!1&&(n.speed=a.ballSpeed,R=!0);let r=n.y-(s.y+s.height/2);r=r/(s.height/2);const i=r*Math.PI/4,l=n.x<e.width/2?1:-1;n.velocityX=l*n.speed*Math.cos(i),n.velocityY=n.speed*Math.sin(i),n.speed+=.1,l===1?n.x=s.x+s.width+n.radius:n.x=s.x-n.radius}if(n.x-n.radius<0?(o.score++,z(e,n)):n.x+n.radius>e.width&&(t.score++,z(e,n)),t.score===a.scoreLimit||o.score===a.scoreLimit){v=f.GAME_OVER;let r;t.score===a.scoreLimit?r="PLAYER 1 WINS!!!":a.multiplayer?r="PLAYER 2 WINS!!!":r="PLAYER 1 LOSES!!!",B(1,[{text:"Restart",onClick:()=>{I(e,t,o,n)}}]),Ee(r),setTimeout(()=>{Ce(t.score,o.score,a.multiplayer)},16)}}function Fe(){L!==null&&(cancelAnimationFrame(L),L=null)}const De=document.querySelector("#app"),F={sidebarOpen:!1};class Ae{constructor(){this._isLoggedIn=!1,this.listeners=[]}get isLoggedIn(){return this._isLoggedIn}get username(){return this._username}get avatarUrl(){return this._avatarUrl}setState(t){let o=!1;t.isLoggedIn!==void 0&&t.isLoggedIn!==this._isLoggedIn&&(this._isLoggedIn=t.isLoggedIn,o=!0),t.username!==void 0&&t.username!==this._username&&(this._username=t.username,o=!0),t.avatarUrl!==void 0&&t.avatarUrl!==this._avatarUrl&&(this._avatarUrl=t.avatarUrl,o=!0),o&&this.listeners.forEach(n=>n())}subscribe(t){this.listeners.push(t)}}const E=new Ae;function T(e){E.setState(e)}async function k(e){De.innerHTML=`
    ${K}
    ...
    ${se}
    ${re}
  `,ge(),fe(),G(t=>{const o=document.getElementById("chat-messages");if(!o)return;const n=document.createElement("div"),s=r=>{if(r==null)return"";if(typeof r=="string")try{const i=JSON.parse(r);return String(i&&typeof i=="object"?i.chat??i.message??JSON.stringify(i):i)}catch{return r}if(typeof r=="object"){const i=r;return String(i.chat??i.message??JSON.stringify(i))}return String(r)};n.textContent=s(t),o.appendChild(n),o.scrollTop=o.scrollHeight}),ye(),Se(),xe(),we()}function W(){switch(window.location.hash.slice(1)||"home"){case"about":k(X);break;case"chat":k(Z);break;case"contact":k(Q);break;case"stats":k(ee);break;case"userSettings":k(te);break;default:k(J)}F.sidebarOpen=!1}window.addEventListener("DOMContentLoaded",async()=>{await ce(),W()});window.addEventListener("hashchange",W);
