...
</section>`,te=`<div id="login-modal" class="fixed inset-0 flex flex-col items-end justify-start mt-22 mr-4 h-screen hidden">
...
`;function le(){try{return JSON.parse(localStorage.getItem("userSession")||"null")}catch{return null}}function $(e,n){localStorage.setItem("userSession",JSON.stringify({token:e,user:n}))}function de(){localStorage.removeItem("userSession")}async function ce(){const e=le();if(e?.token)try{const n=await fetch("undefined/me",{headers:{Authorization:`Bearer ${e.token}`}});if(!n.ok){console.warn("Stored session invalid, clearing..."),de();return}const o=await n.json();N({isLoggedIn:!0,username:o.username,avatarUrl:"/default-avatar.png"}),console.log("✅ Session restored:",o.username)}catch(n){console.error("Error verifying stored session:",n)}}const Y=`<div class="flex flex-row gap-2 justify-center items-center">
    ...
    <div class="w-2 h-2 rounded-full bg-black animate-bounce [animation-delay:-.5s]"></div>
</div>`,ue="undefined/auth/login";function pe(){const e=document.getElementById("login-modal"),n=document.getElementById("signup-modal"),o=document.getElementById("open-login"),t=document.getElementById("open-signup"),s=document.getElementById("login-form"),r=document.getElementById("login-email"),i=document.getElementById("login-password"),l=document.getElementById("login-submit"),p=document.getElementById("login-message");if(!e||!o||!s||!r||!i||!l){console.warn("Login modal setup failed: missing elements.");return}o.addEventListener("click",()=>e.classList.toggle("hidden")),e.addEventListener("click",v=>{v.target===e&&e.classList.add("hidden")}),t&&n&&t.addEventListener("click",()=>{e.classList.add("hidden"),n.classList.remove("hidden")}),s.addEventListener("submit",async v=>{v.preventDefault();const m=l.innerHTML;l.innerHTML=Y,l.disabled=!0;try{const y=await fetch(ue,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:r.value,password:i.value})});if(!y.ok){p&&(p.textContent="Invalid email or password"),l.innerHTML=m,l.disabled=!1;return}const d=await y.json();d.token&&d.user&&($(d.token,{username:d.user.username,avatarUrl:"/default-avatar.png"}),N({isLoggedIn:!0,username:d.user.username,avatarUrl:"/default-avatar.png"})),e.classList.add("hidden")}catch(y){console.error("Login error:",y),p&&(p.textContent="An error occurred. Please try again.")}finally{l.innerHTML=m,l.disabled=!1}})}const me="undefined/auth/register";function he(){const e=document.getElementById("signup-modal"),n=document.getElementById("open-login"),o=document.getElementById("signup-form"),t=document.getElementById("signup-username"),s=document.getElementById("signup-email"),r=document.getElementById("signup-comfirmEmail"),i=document.getElementById("signup-password"),l=document.getElementById("signup-confirmPassword"),p=o?.querySelector('button[type="submit"]');if(!e||!o||!t||!s||!r||!i||!l||!p){console.warn("Signup modal setup failed: missing elements.");return}function v(d){return d.length>=8&&/[A-Z]/.test(d)&&/[0-9]/.test(d)}function m(d,u,b,g){u?(d.classList.remove("border-red-500","focus:ring-red-500"),d.classList.add("border-green-500","focus:ring-green-500")):(d.classList.remove("border-green-500","focus:ring-green-500"),d.classList.add("border-red-500","focus:ring-red-500"))}function y(){let d=!0;t.value.trim()?m(t,!0):(d=!1,m(t,!1)),s.value.trim()?m(s,!0):(d=!1,m(s,!1)),r.value.trim()!==s.value.trim()?(d=!1,m(r,!1)):m(r,!0),v(i.value)?m(i,!0):(d=!1,m(i,!1)),l.value!==i.value?(d=!1,m(l,!1)):m(l,!0),p.disabled=!d}[t,s,r,i,l].forEach(d=>d?.addEventListener("input",y)),n&&n.addEventListener("click",()=>e.classList.add("hidden")),e.addEventListener("click",d=>{d.target===e&&e.classList.add("hidden")}),o.addEventListener("submit",async d=>{d.preventDefault();const u=t.value.trim(),b=s.value.trim(),g=i.value.trim(),w=p.innerHTML;p.innerHTML=Y,p.disabled=!0;try{const c=new AbortController,q=setTimeout(()=>c.abort(),15e3),T=await fetch(me,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:u,email:b,password:g}),signal:c.signal});if(clearTimeout(q),!T.ok){alert("Signup failed. Please try again."),i.value="",l.value="";return}const E=await T.json();E.token&&$(E.token,{username:E.username||u,avatarUrl:E.avatarUrl||"/default-avatar.png"}),N({isLoggedIn:!0,username:E.username||u,avatarUrl:E.avatarUrl||"/default-avatar.png"}),e.classList.add("hidden"),console.log("Signup successful!")}catch(c){console.error("Signup error:",c),alert("Network error or request timeout.")}finally{p.innerHTML=w,p.disabled=!1}})}async function ge(){const e=document.getElementById("user-section");if(!e){console.warn("⚠️ user-section not found in DOM");return}function n(){if(S.isLoggedIn){e.innerHTML=ie;const o=document.getElementById("user-greeting");o&&(o.textContent=S.username||"User");const t=document.getElementById("user-avatar"),s=document.getElementById("default-avatar");S.avatarUrl?(t&&(t.src=S.avatarUrl,t.classList.remove("hidden")),s&&s.classList.add("hidden")):(t&&t.classList.add("hidden"),s&&s.classList.remove("hidden"));const r=document.getElementById("logout-btn");r&&r.addEventListener("click",()=>{S.setState({isLoggedIn:!1,username:void 0,avatarUrl:void 0})})}else e.innerHTML=ae,pe(),he()}n(),S.subscribe(n)}function fe(){const e=document.getElementById("menu-toggle"),n=document.getElementById("sidebar"),o=document.getElementById("page-content");!e||!n||!o||e.addEventListener("change",()=>{U.sidebarOpen=e.checked,U.sidebarOpen?(n.classList.remove("-translate-x-full"),o.classList.add("ml-48")):(n.classList.add("-translate-x-full"),o.classList.remove("ml-48"))})}let x=null;function G(e){const n=`ws://${window.location.hostname}:3000/ws`;return(!x||x.readyState===WebSocket.CLOSED||x.readyState===WebSocket.CLOSING)&&(x=new WebSocket(n),x.onopen=()=>{console.log("Connected to WebSocket:",n)},x.onclose=()=>{console.log("Disconnected from server!"),x=null},x.onerror=o=>{console.log("WebSocket error!",o),x=null}),x.onmessage=o=>{if(e)try{e(JSON.parse(o.data))}catch{console.warn("Received non-JSON message:",o.data)}},x}function be(e){x&&x.readyState===WebSocket.OPEN?x.send(JSON.stringify(e)):console.warn("WebSocket not connected!")}function ve(){const e=localStorage.getItem("username");return e||(window.anonymousId||(window.anonymousId="Anonymous"+Math.floor(1e3+Math.random()*9e3)),window.anonymousId)}function ye(){const e=document.querySelector("#chat-input"),n=document.querySelector("#chat-send"),o=document.getElementById("chat-messages");if(o&&(o.innerHTML=""),G(t=>{if(o)if(console.log("Received message:",t),t.type==="chat"){const s=document.createElement("div");s.className="mb-2 p-2 rounded bg-gray-100";const r=t.username||"Anonymous",i=t.text||"";s.textContent=`${r}: ${i}`,o.appendChild(s),o.scrollTop=o.scrollHeight}else if(t.type==="welcome"){const s=document.createElement("div");s.className="mb-2 p-2 rounded bg-blue-100 text-blue-800",s.textContent=t.message,o.appendChild(s),o.scrollTop=o.scrollHeight}else console.warn("Unknown message type:",t)}),e&&n){const t=()=>{e.value.trim()&&(be({type:"chat",text:e.value.trim(),username:ve()}),e.value="")};n.addEventListener("click",t),e.addEventListener("keypress",s=>{s.key==="Enter"&&t()})}}let a={difficulty:.05,mouse:!0,multiplayer:!1,resetSpeed:!0,paddleSpeed:5,ballSpeed:3,scoreLimit:5,bgColor:"#1C39BB",itemsColor:"#F5CB5C",default:!1};function xe(){const e=document.getElementById("openSettings"),n=document.getElementById("closeSettings"),o=document.getElementById("controlPanel"),t=document.getElementById("paddleSpeed"),s=document.getElementById("paddleSpeedValue"),r=document.getElementById("ballSpeed"),i=document.getElementById("ballSpeedValue"),l=document.getElementById("scoreLimit"),p=document.getElementById("bgColor"),v=document.getElementById("itemsColor"),m=document.getElementById("resetSpeed"),y=document.getElementById("resetDefaults"),d=document.querySelectorAll('input[name="difficulty"]'),u=document.querySelectorAll('input[name="mouse"]'),b=document.querySelectorAll('input[name="multiplayer"]');function g(){d.forEach(c=>{c.checked=Number(c.value)===a.difficulty}),u.forEach(c=>{c.checked=c.value==="true"===a.mouse}),b.forEach(c=>{c.checked=c.value==="true"===a.multiplayer}),m.checked=a.resetSpeed,t.value=a.paddleSpeed.toString(),s.textContent=t.value,r.value=a.ballSpeed.toString(),i.textContent=r.value,l.value=a.scoreLimit.toString(),p.value=a.bgColor,v.value=a.itemsColor}function w(){d.forEach(c=>{c.addEventListener("change",()=>{a.difficulty=Number(c.value)})}),u.forEach(c=>{c.addEventListener("change",()=>{a.multiplayer===!1&&(a.mouse=c.value==="true")})}),b.forEach(c=>{c.addEventListener("change",()=>{a.multiplayer=c.value==="true",a.mouse=!1})}),m.addEventListener("change",()=>{a.resetSpeed=m.checked}),t.addEventListener("input",()=>{a.paddleSpeed=Number(t.value),s.textContent=t.value}),r.addEventListener("input",()=>{a.ballSpeed=Number(r.value),i.textContent=r.value}),l.addEventListener("input",()=>{const c=Number(l.value);c>0&&(a.scoreLimit=c)}),p.addEventListener("input",()=>{a.bgColor=p.value}),v.addEventListener("input",()=>{a.itemsColor=v.value}),y.addEventListener("click",()=>{a={...a,difficulty:.05,mouse:!1,resetSpeed:!0,paddleSpeed:5,ballSpeed:3,scoreLimit:5,bgColor:"#1C39BB",itemsColor:"#F5CB5C",default:!0},g()})}e?.addEventListener("click",()=>{g(),o?.classList.toggle("hidden")}),n?.addEventListener("click",()=>{o?.classList.add("hidden")}),w()}function we(){if(!document.getElementById("stats-container"))return;const n=document.getElementById("fetchData"),o=document.getElementById("usernameInput"),t=document.getElementById("errorMessage"),s=document.getElementById("loading"),r=document.getElementById("infoUsername"),i=document.getElementById("infoEmail"),l=document.getElementById("winsCount"),p=document.getElementById("lossesCount"),v=document.getElementById("winRatio");async function m(u){s.style.display="block",t.style.display="none";try{const b=await fetch(`/stats/api/user-stats?username=${u}`),g=await fetch(`/stats/api/matches/${u}`);if(!b.ok||!g.ok)throw new Error("Invalid user");const w=await b.json(),c=await g.json();y(w,c)}catch(b){t.textContent=`Error: ${b.message}`,t.style.display="block"}finally{s.style.display="none"}}function y(u,b){r.textContent=u.username,i.textContent=u.email;let g=0,w=0;b.matches.forEach(c=>{c.winner===u.username?g++:c.loser===u.username&&w++}),l.textContent=String(g),p.textContent=String(w),v.textContent=`${(g/(g+w)*100||0).toFixed(1)}%`,d(b)}function d(u){const b=document.getElementById("matchHistoryGrid");if(b.innerHTML="",!u.matches||u.matches.length===0){b.innerHTML="<p>No matches yet</p>";return}u.matches.forEach(g=>{const w=document.createElement("div");w.classList.add("match-card"),w.innerHTML=`
        <h3>Match #${g.id}</h3>
        <div class="match-details">
      ...
      `,b.appendChild(w)})}n.addEventListener("click",()=>{const u=o.value.trim();u?m(u):(t.textContent="Please enter a username",t.style.display="block")})}async function Ce(e,n,o){var t,s,r,i;e>n?(r=e,i=n,t="nome",o?s="guest_multiplayer":s="bot"):(r=n,i=e,s="nome",o?t="guest_multiplayer":t="bot");try{const l=await fetch("/stats/api/match",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({winner:t,loser:s,winner_points:r,loser_points:i})});if(!l.ok){const v=await l.json();console.error("Failed to save match:",v);return}return await l.json()}catch(l){console.error("Error saving match:",l)}}let H=!1,R=!1;const h={START:"start",PLAYING:"playing",PAUSED:"paused",GAME_OVER:"gameOver"};let f=h.START,L=null,_=!1,F=!1,D=!1,A=!1;window.addEventListener("keydown",e=>{(e.key==="w"||e.key==="W")&&(_=!0),(e.key==="s"||e.key==="S")&&(F=!0),e.key==="ArrowUp"&&(D=!0),e.key==="ArrowDown"&&(A=!0)});window.addEventListener("keyup",e=>{(e.key==="w"||e.key==="W")&&(_=!1),(e.key==="s"||e.key==="S")&&(F=!1),e.key==="ArrowUp"&&(D=!1),e.key==="ArrowDown"&&(A=!1)});function Se(e){const n=document.getElementById("game-overlay"),o=document.getElementById("game-message");!n||!o||(o.textContent=e,n.classList.remove("hidden"))}function B(e,n){const o=document.getElementById("game-overlay"),t=document.getElementById("overlay-buttons");!o||!t||(t.innerHTML="",e===1&&n.forEach(s=>{const r=document.createElement("button");r.textContent=s.text,r.className="px-8 py-2 text-white font-lucky text-lg rounded-full shadow-lg transition-transform 							transform bg-[#00091D] border-2 border-white hover:scale-105 hover:border-green-600 							hover:shadow-green-500/50 hover:shadow-2xl focus:outline-none hover:text-green-600",r.onclick=s.onClick,t.appendChild(r)}),e===2&&n.forEach(s=>{const r=document.createElement("button");r.textContent=s.text,r.className="w-35 px-8 py-2 text-white font-lucky text-lg rounded-lg shadow-lg transition-transform 							transform bg-[#00091D] border-2 border-white hover:scale-105 hover:border-blue-600 							hover:shadow-blue-500/50 hover:shadow-2xl focus:outline-none hover:text-blue-600",r.onclick=s.onClick,t.appendChild(r)}),o.classList.remove("hidden"))}function C(){const e=document.getElementById("game-overlay");e&&e.classList.add("hidden")}function Ee(){Ue();const{canvas:e,context:n}=Ie();if(!e||!n)return;const o={x:30,y:e.height/2-100/2,width:10,height:100,score:0},t={x:e.width-40,y:e.height/2-100/2,width:10,height:100,score:0},s={x:e.width/2,y:e.height/2,radius:10,velocityX:0,velocityY:0,speed:a.ballSpeed},r={x:e.width/2-1,y:0,width:2,height:10};o.y=e.height/2-o.height/2,t.y=e.height/2-t.height/2,s.x=e.width/2,s.y=e.height/2,s.velocityX=0,s.velocityY=0;const i=document.getElementById("pause-btn"),l=document.getElementById("openSettings"),p=y=>{ke(y,e,o)};a.mouse&&!a.multiplayer&&e.addEventListener("mousemove",p),B(1,[{text:"Start",onClick:()=>{f=h.PLAYING,C(),P(s)}}]),i&&i.addEventListener("click",()=>{f===h.PLAYING&&(f=h.PAUSED,B(2,[{text:"Resume",onClick:()=>{f=h.PLAYING,C()}},{text:"Restart",onClick:()=>{I(e,o,t,s)}}]))}),l&&l.addEventListener("click",()=>{f===h.PLAYING&&(f=h.PAUSED,B(2,[{text:"Resume",onClick:()=>{f=h.PLAYING,C()}},{text:"Restart",onClick:()=>{I(e,o,t,s)}}]))}),window.addEventListener("keydown",y=>{y.code==="Space"&&(f===h.PLAYING?(f=h.PAUSED,B(2,[{text:"Resume",onClick:()=>{f=h.PLAYING,C()}},{text:"Restart",onClick:()=>{I(e,o,t,s)}}])):f===h.PAUSED?(f=h.PLAYING,C()):f===h.START&&(f=h.PLAYING,C(),P(s)))});function v(){if(f===h.START||f===h.PAUSED||f===h.GAME_OVER){j(e,n,o,t,s,r);return}a.multiplayer&&Be(e,o,t),!a.multiplayer&&!a.mouse&&Le(e,o),Fe(e,o,t,s),j(e,n,o,t,s,r)}function m(){(!a.mouse||a.multiplayer)&&e.removeEventListener("mousemove",p),v(),L=requestAnimationFrame(m)}L=requestAnimationFrame(m)}function ke(e,n,o){const t=n.getBoundingClientRect();let s=e.clientY-t.top-o.height/2;s<0&&(s=0),s+o.height>n.height&&(s=n.height-o.height),o.y=s}function I(e,n,o,t){n.score=0,o.score=0,n.y=e.height/2-n.height/2,o.y=e.height/2-o.height/2,t.x=e.width/2,t.y=e.height/2,t.velocityX=0,t.velocityY=0,f=h.PLAYING,C(),P(t)}function P(e){let n=e.speed;H===!1&&(n=a.ballSpeed,H=!0);const o=Math.random()*Math.PI/2-Math.PI/4,t=Math.random()<.5?1:-1;e.velocityX=t*n*Math.cos(o),e.velocityY=n*Math.sin(o)}function Be(e,n,o){_&&(n.y-=a.paddleSpeed),F&&(n.y+=a.paddleSpeed),D&&(o.y-=a.paddleSpeed),A&&(o.y+=a.paddleSpeed),n.y=Math.max(0,Math.min(n.y,e.height-n.height)),o.y=Math.max(0,Math.min(o.y,e.height-o.height))}function Le(e,n){_&&(n.y-=a.paddleSpeed),F&&(n.y+=a.paddleSpeed),n.y=Math.max(0,Math.min(n.y,e.height-n.height))}function Ie(){const e=document.getElementById("pong");return e?{canvas:e,context:e.getContext("2d")}:{canvas:null,context:null}}function M(e,n,o,t,s,r,i=!1,l="none"){e.fillStyle=r,e.fillRect(n,o,t,s),i&&(e.strokeStyle=l,e.lineWidth=2,e.strokeRect(0,0,t,s))}function V(e,n,o,t,s){e.fillStyle=s,e.beginPath(),e.arc(n,o,t,0,Math.PI*2,!1),e.closePath(),e.fill()}function O(e,n,o,t,s){e.fillStyle=s,e.font="45px fantasy",e.fillText(n.toString(),o,t)}function Me(e,n,o){V(n,o.x,0,30,a.itemsColor);for(let t=0;t<=e.height;t+=15)M(n,o.x,o.y+t,o.width,o.height,a.itemsColor)}function j(e,n,o,t,s,r){M(n,0,0,e.width,e.height,a.bgColor,!0,a.itemsColor),Me(e,n,r),O(n,o.score,e.width/4,e.height/5,a.itemsColor),O(n,t.score,3*e.width/4,e.height/5,a.itemsColor),M(n,o.x,o.y,o.width,o.height,a.itemsColor),M(n,t.x,t.y,t.width,t.height,a.itemsColor),V(n,s.x,s.y,s.radius,a.itemsColor)}function Pe(e,n){return e.top=e.y-e.radius,e.bottom=e.y+e.radius,e.left=e.x-e.radius,e.right=e.x+e.radius,n.top=n.y,n.bottom=n.y+n.height,n.left=n.x,n.right=n.x+n.width,e.right>n.left&&e.bottom>n.top&&e.left<n.right&&e.top<n.bottom}function z(e,n){n.x=e.width/2,n.y=e.height/2,a.resetSpeed&&(n.speed=a.ballSpeed),P(n)}function _e(e,n,o){const t=n.y+n.height/2;n.y+=(o.y-t)*a.difficulty,n.y=Math.max(0,Math.min(n.y,e.height-n.height))}function Fe(e,n,o,t){t.x+=t.velocityX,t.y+=t.velocityY,a.multiplayer||_e(e,o,t),t.y+t.radius>e.height?(t.y=e.height-t.radius,t.velocityY=-t.velocityY):t.y-t.radius<0&&(t.y=t.radius,t.velocityY=-t.velocityY);const s=t.x<e.width/2?n:o;if(Pe(t,s)){R===!1&&(t.speed=a.ballSpeed,R=!0);let r=t.y-(s.y+s.height/2);r=r/(s.height/2);const i=r*Math.PI/4,l=t.x<e.width/2?1:-1;t.velocityX=l*t.speed*Math.cos(i),t.velocityY=t.speed*Math.sin(i),t.speed+=.1,l===1?t.x=s.x+s.width+t.radius:t.x=s.x-t.radius}if(t.x-t.radius<0?(o.score++,z(e,t)):t.x+t.radius>e.width&&(n.score++,z(e,t)),n.score===a.scoreLimit||o.score===a.scoreLimit){f=h.GAME_OVER;let r;n.score===a.scoreLimit?r="PLAYER 1 WINS!!!":a.multiplayer?r="PLAYER 2 WINS!!!":r="PLAYER 1 LOSES!!!",B(1,[{text:"Restart",onClick:()=>{I(e,n,o,t)}}]),Se(r),setTimeout(()=>{Ce(n.score,o.score,a.multiplayer)},16)}}function Ue(){L!==null&&(cancelAnimationFrame(L),L=null)}const De=document.querySelector("#app"),U={sidebarOpen:!1};class Ae{constructor(){this._isLoggedIn=!1,this.listeners=[]}get isLoggedIn(){return this._isLoggedIn}get username(){return this._username}get avatarUrl(){return this._avatarUrl}setState(n){let o=!1;n.isLoggedIn!==void 0&&n.isLoggedIn!==this._isLoggedIn&&(this._isLoggedIn=n.isLoggedIn,o=!0),n.username!==void 0&&n.username!==this._username&&(this._username=n.username,o=!0),n.avatarUrl!==void 0&&n.avatarUrl!==this._avatarUrl&&(this._avatarUrl=n.avatarUrl,o=!0),o&&this.listeners.forEach(t=>t())}subscribe(n){this.listeners.push(n)}}const S=new Ae;function N(e){S.setState(e)}async function k(e){De.innerHTML=`
    ${K}
    ...
    ${se}
    ${re}
  `,ge(),fe(),G(n=>{const o=document.getElementById("chat-messages");if(!o)return;const t=document.createElement("div"),s=r=>{if(r==null)return"";if(typeof r=="string")try{const i=JSON.parse(r);return String(i&&typeof i=="object"?i.chat??i.message??JSON.stringify(i):i)}catch{return r}if(typeof r=="object"){const i=r;return String(i.chat??i.message??JSON.stringify(i))}return String(r)};t.textContent=s(n),o.appendChild(t),o.scrollTop=o.scrollHeight}),ye(),Ee(),xe(),we()}function W(){switch(window.location.hash.slice(1)||"home"){case"about":k(X);break;case"chat":k(Z);break;case"contact":k(Q);break;case"stats":k(ee);break;case"userSettings":k(ne);break;default:k(J)}U.sidebarOpen=!1}window.addEventListener("DOMContentLoaded",async()=>{await ce(),W()});window.addEventListener("hashchange",W);
