...
<head>
    ...
    <style>
        #stats-container {
            ...
            min-height: 100vh;
            padding: 20px;
            background: linear-gradient(180deg, #0E172B, #1a2a6c, #274690);
        }
...
`,G=`<div class="flex flex-row gap-2 justify-center items-center">
    ...
    <div class="w-2 h-2 rounded-full bg-black animate-bounce [animation-delay:-.5s]"></div>
</div>`,ue="undefined/auth/login";function pe(){const e=document.getElementById("login-modal"),n=document.getElementById("signup-modal"),o=document.getElementById("open-login"),t=document.getElementById("open-signup"),s=document.getElementById("login-form"),r=document.getElementById("login-email"),i=document.getElementById("login-password"),l=document.getElementById("login-submit"),u=document.getElementById("login-message");if(!e||!o||!s||!r||!i||!l){console.warn("Login modal setup failed: missing elements.");return}o.addEventListener("click",()=>{e.classList.toggle("hidden")}),e.addEventListener("click",c=>{c.target===e&&e.classList.add("hidden")}),t&&n&&t.addEventListener("click",()=>{e.classList.add("hidden"),n.classList.remove("hidden")});function b(){r.value.trim()&&i.value.trim()?l.disabled=!1:l.disabled=!0}r.addEventListener("input",b),i.addEventListener("input",b);function x(c){u&&(u.textContent=c,u.classList.remove("hidden"),u.classList.toggle("text-red-500"))}function E(){u&&u.classList.add("hidden")}s.addEventListener("submit",async c=>{c.preventDefault(),E();const g=l.innerHTML;l.innerHTML=`${G}`,l.disabled=!0;const h=new AbortController,m=setTimeout(()=>h.abort(),15e3);try{const d=await fetch(ue,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:r.value,password:i.value}),signal:h.signal});if(clearTimeout(m),!d.ok){x("Invalid email or password"),i.value="",b(),l.innerHTML=g,l.disabled=!1;return}const p=await d.json();y.isLoggedIn=!0,y.username=p.username,y.avatarUrl=p.avatarUrl,e.classList.add("hidden"),r.value="",i.value="",l.innerHTML=g,l.disabled=!0,V()}catch{h.signal.aborted?(x("Login request timed out. Please try again."),r.value="",i.value=""):x("An error occurred. Please try again."),l.innerHTML=g,l.disabled=!0}}),document.getElementById("forgotPass").addEventListener("click",()=>{y.isLoggedIn=!0,e.classList.add("hidden")})}const me="undefined/auth/register";function he(){const e=document.getElementById("signup-modal"),n=document.getElementById("open-login"),o=document.getElementById("signup-form"),t=document.getElementById("signup-username"),s=document.getElementById("signup-email"),r=document.getElementById("signup-comfirmEmail"),i=document.getElementById("signup-password"),l=document.getElementById("signup-confirmPassword"),u=o?.querySelector('button[type="submit"]');if(!e||!o||!t||!s||!r||!i||!l||!u){console.warn("Signup modal setup failed: missing elements.");return}function b(p){let C=p.nextElementSibling;return(!C||!C.classList.contains("error-msg"))&&(C=document.createElement("span"),C.className="error-msg text-red-500 text-sm block hidden",p.insertAdjacentElement("afterend",C)),C}const x=b(t),E=b(s),B=b(r),c=b(i),g=b(l);u.disabled=!0;function h(p){const C=p.length>=8,S=/[A-Z]/.test(p),k=/[0-9]/.test(p);return C&&S&&k}function m(p,C,S,k){C?(p.classList.remove("border-red-500","focus:ring-red-500"),p.classList.add("border-green-500","focus:ring-green-500"),k&&k.classList.add("hidden")):(p.classList.remove("border-green-500","focus:ring-green-500"),p.classList.add("border-red-500","focus:ring-red-500"),k&&S&&(k.textContent=S,k.classList.remove("hidden")))}function d(){let p=!0;t.value.trim().length===0?(m(t,!1,"Username is required.",x),p=!1):m(t,!0,"",x),s.value.trim().length===0?(m(s,!1,"Email is required.",E),p=!1):m(s,!0,"",E),r.value.trim()!==s.value.trim()||r.value.trim()===""?(m(r,!1,"Emails do not match.",B),p=!1):m(r,!0,"",B),h(i.value)?m(i,!0,"",c):(m(i,!1,"Password must be ≥ 8 chars, 1 uppercase & 1 number.",c),p=!1),l.value!==i.value||l.value===""?(m(l,!1,"Passwords do not match.",g),p=!1):m(l,!0,"",g),u.disabled=!p}t.addEventListener("input",d),s.addEventListener("input",d),r.addEventListener("input",d),i.addEventListener("input",d),l.addEventListener("input",d),n&&n.addEventListener("click",()=>{e.classList.add("hidden")}),e.addEventListener("click",p=>{p.target===e&&e.classList.add("hidden")}),o.addEventListener("submit",async p=>{p.preventDefault();const C=t.value.trim(),S=s.value.trim(),k=i.value.trim(),J=u.innerHTML;u.innerHTML=G,u.disabled=!0;try{const P=new AbortController,K=setTimeout(()=>P.abort(),15e3),R=await fetch(me,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:C,email:S,password:k}),signal:P.signal});if(clearTimeout(K),R.ok){const X=await R.json();console.log("Signup response:",X),y.isLoggedIn=!0,y.username=C,y.avatarUrl="/default-avatar.png",e.classList.add("hidden"),console.log("Signup successful!")}else alert("Signup failed. Please try again."),i.value="",l.value=""}catch(P){console.error("Signup error:",P),alert("Network error or request timeout."),i.value="",l.value=""}finally{u.innerHTML=J,u.disabled=!1}})}async function V(){const e=document.getElementById("user-section");if(!e){console.warn("⚠️ user-section not found in DOM");return}function n(){if(y.isLoggedIn){e.innerHTML=ce;const o=document.getElementById("user-greeting");o&&(o.textContent=y.username?` ${y.username}`:" User");const t=document.getElementById("user-avatar"),s=document.getElementById("default-avatar"),r=document.getElementById("user-avatar-modal"),i=document.getElementById("default-avatar-modal");y.avatarUrl?(t&&(t.src=y.avatarUrl,t.classList.remove("hidden")),s&&s.classList.add("hidden"),r&&(r.src=y.avatarUrl,r.classList.remove("hidden")),i&&i.classList.add("hidden")):(t&&t.classList.add("hidden"),s&&s.classList.remove("hidden"),r&&r.classList.add("hidden"),i&&i.classList.remove("hidden"));const l=document.getElementById("logout-btn");l&&l.addEventListener("click",()=>{y.isLoggedIn=!1,y.username=void 0,y.avatarUrl=void 0})}else e.innerHTML=de,pe(),he()}n(),y.subscribe&&y.subscribe(n)}function ge(){const e=document.getElementById("menu-toggle"),n=document.getElementById("sidebar"),o=document.getElementById("page-content");!e||!n||!o||e.addEventListener("change",()=>{U.sidebarOpen=e.checked,U.sidebarOpen?(n.classList.remove("-translate-x-full"),o.classList.add("ml-48")):(n.classList.add("-translate-x-full"),o.classList.remove("ml-48"))})}let w=null;function fe(e){const n=`ws://${window.location.hostname}:3000/ws`;return(!w||w.readyState===WebSocket.CLOSED||w.readyState===WebSocket.CLOSING)&&(w=new WebSocket(n),w.onopen=()=>{console.log("Connected to WebSocket:",n)},w.onclose=()=>{console.log("Disconnected from server!"),w=null},w.onerror=o=>{console.log("WebSocket error!",o),w=null}),w.onmessage=o=>{if(e)try{e(JSON.parse(o.data))}catch{console.warn("Received non-JSON message:",o.data)}},w}function be(e){w&&w.readyState===WebSocket.OPEN?w.send(JSON.stringify(e)):console.warn("WebSocket not connected!")}function ve(){const e=localStorage.getItem("username");return e||(window.anonymousId||(window.anonymousId="Anonymous"+Math.floor(1e3+Math.random()*9e3)),window.anonymousId)}function ye(){const e=document.querySelector("#chat-input"),n=document.querySelector("#chat-send"),o=document.getElementById("chat-messages");if(o&&(o.innerHTML=""),fe(t=>{if(o)if(console.log("Received message:",t),t.type==="chat"){const s=document.createElement("div");s.className="mb-2 p-2 rounded bg-gray-100";const r=t.username||"Anonymous",i=t.text||"";s.textContent=`${r}: ${i}`,o.appendChild(s),o.scrollTop=o.scrollHeight}else if(t.type==="welcome"){const s=document.createElement("div");s.className="mb-2 p-2 rounded bg-blue-100 text-blue-800",s.textContent=t.message,o.appendChild(s),o.scrollTop=o.scrollHeight}else console.warn("Unknown message type:",t)}),e&&n){const t=()=>{e.value.trim()&&(be({type:"chat",text:e.value.trim(),username:ve()}),e.value="")};n.addEventListener("click",t),e.addEventListener("keypress",s=>{s.key==="Enter"&&t()})}}let a={difficulty:.05,mouse:!0,multiplayer:!1,resetSpeed:!0,paddleSpeed:5,ballSpeed:3,scoreLimit:5,bgColor:"#1C39BB",itemsColor:"#F5CB5C",default:!1};function xe(){const e=document.getElementById("openSettings"),n=document.getElementById("closeSettings"),o=document.getElementById("controlPanel"),t=document.getElementById("paddleSpeed"),s=document.getElementById("paddleSpeedValue"),r=document.getElementById("ballSpeed"),i=document.getElementById("ballSpeedValue"),l=document.getElementById("scoreLimit"),u=document.getElementById("bgColor"),b=document.getElementById("itemsColor"),x=document.getElementById("resetSpeed"),E=document.getElementById("resetDefaults"),B=document.querySelectorAll('input[name="difficulty"]'),c=document.querySelectorAll('input[name="mouse"]'),g=document.querySelectorAll('input[name="multiplayer"]');function h(){B.forEach(d=>{d.checked=Number(d.value)===a.difficulty}),c.forEach(d=>{d.checked=d.value==="true"===a.mouse}),g.forEach(d=>{d.checked=d.value==="true"===a.multiplayer}),x.checked=a.resetSpeed,t.value=a.paddleSpeed.toString(),s.textContent=t.value,r.value=a.ballSpeed.toString(),i.textContent=r.value,l.value=a.scoreLimit.toString(),u.value=a.bgColor,b.value=a.itemsColor}function m(){B.forEach(d=>{d.addEventListener("change",()=>{a.difficulty=Number(d.value)})}),c.forEach(d=>{d.addEventListener("change",()=>{a.multiplayer===!1&&(a.mouse=d.value==="true")})}),g.forEach(d=>{d.addEventListener("change",()=>{a.multiplayer=d.value==="true",a.mouse=!1})}),x.addEventListener("change",()=>{a.resetSpeed=x.checked}),t.addEventListener("input",()=>{a.paddleSpeed=Number(t.value),s.textContent=t.value}),r.addEventListener("input",()=>{a.ballSpeed=Number(r.value),i.textContent=r.value}),l.addEventListener("input",()=>{const d=Number(l.value);d>0&&(a.scoreLimit=d)}),u.addEventListener("input",()=>{a.bgColor=u.value}),b.addEventListener("input",()=>{a.itemsColor=b.value}),E.addEventListener("click",()=>{a={...a,difficulty:.05,mouse:!1,resetSpeed:!0,paddleSpeed:5,ballSpeed:3,scoreLimit:5,bgColor:"#1C39BB",itemsColor:"#F5CB5C",default:!0},h()})}e?.addEventListener("click",()=>{h(),o?.classList.toggle("hidden")}),n?.addEventListener("click",()=>{o?.classList.add("hidden")}),m()}function we(){if(!document.getElementById("stats-container"))return;console.log("Stats Script loaded");const n=document.getElementById("fetchData"),o=document.getElementById("usernameInput"),t=document.getElementById("errorMessage"),s=document.getElementById("loading"),r=document.getElementById("infoUsername"),i=document.getElementById("infoEmail"),l=document.getElementById("winsCount"),u=document.getElementById("lossesCount"),b=document.getElementById("winRatio");async function x(c){s.style.display="block",t.style.display="none";try{const g=await fetch(`/stats/api/user-stats?username=${c}`),h=await fetch(`/stats/api/matches/${c}`);if(!g.ok||!h.ok)throw new Error("Invalid user");const m=await g.json(),d=await h.json();E(m,d)}catch(g){t.textContent=`Error: ${g.message}`,t.style.display="block"}finally{s.style.display="none"}}function E(c,g){r.textContent=c.username,i.textContent=c.email;let h=0,m=0;g.matches.forEach(d=>{d.winner===c.username?h++:d.loser===c.username&&m++}),l.textContent=String(h),u.textContent=String(m),b.textContent=`${(h/(h+m)*100||0).toFixed(1)}%`,B(g)}function B(c){const g=document.getElementById("matchHistoryGrid");if(g.innerHTML="",!c.matches||c.matches.length===0){g.innerHTML="<p>No matches yet</p>";return}c.matches.forEach(h=>{const m=document.createElement("div");m.classList.add("match-card"),m.innerHTML=`
        <h3>Match #${h.id}</h3>
        <div class="match-details">
      ...
      `,g.appendChild(m)})}n.addEventListener("click",()=>{const c=o.value.trim();c?x(c):(t.textContent="Please enter a username",t.style.display="block")})}async function Ce(e,n,o){var t,s,r,i;e>n?(r=e,i=n,t="nome",o?s="guest_multiplayer":s="bot"):(r=n,i=e,s="nome",o?t="guest_multiplayer":t="bot");try{const l=await fetch("/stats/api/match",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({winner:t,loser:s,winner_points:r,loser_points:i})});if(!l.ok){const b=await l.json();console.error("Failed to save match:",b);return}return await l.json()}catch(l){console.error("Error saving match:",l)}}let O=!1,j=!1;const f={START:"start",PLAYING:"playing",PAUSED:"paused",GAME_OVER:"gameOver"};let v=f.START,A=!1,T=!1,N=!1,H=!1;window.addEventListener("keydown",e=>{(e.key==="w"||e.key==="W")&&(A=!0),(e.key==="s"||e.key==="S")&&(T=!0),e.key==="ArrowUp"&&(N=!0),e.key==="ArrowDown"&&(H=!0)});window.addEventListener("keyup",e=>{(e.key==="w"||e.key==="W")&&(A=!1),(e.key==="s"||e.key==="S")&&(T=!1),e.key==="ArrowUp"&&(N=!1),e.key==="ArrowDown"&&(H=!1)});function Ee(e){const n=document.getElementById("game-overlay"),o=document.getElementById("game-message");!n||!o||(o.textContent=e,n.classList.remove("hidden"))}function M(e,n){const o=document.getElementById("game-overlay"),t=document.getElementById("overlay-buttons");!o||!t||(t.innerHTML="",e===1&&n.forEach(s=>{const r=document.createElement("button");r.textContent=s.text,r.className="px-8 py-2 text-white font-lucky text-lg rounded-full shadow-lg transition-transform 							transform bg-[#00091D] border-2 border-white hover:scale-105 hover:border-green-600 							hover:shadow-green-500/50 hover:shadow-2xl focus:outline-none hover:text-green-600",r.onclick=s.onClick,t.appendChild(r)}),e===2&&n.forEach(s=>{const r=document.createElement("button");r.textContent=s.text,r.className="w-35 px-8 py-2 text-white font-lucky text-lg rounded-lg shadow-lg transition-transform 							transform bg-[#00091D] border-2 border-white hover:scale-105 hover:border-blue-600 							hover:shadow-blue-500/50 hover:shadow-2xl focus:outline-none hover:text-blue-600",r.onclick=s.onClick,t.appendChild(r)}),o.classList.remove("hidden"))}function L(){const e=document.getElementById("game-overlay");e&&e.classList.add("hidden")}function ke(){const{canvas:e,context:n}=Ie();if(!e||!n)return;const o={x:30,y:e.height/2-100/2,width:10,height:100,score:0},t={x:e.width-40,y:e.height/2-100/2,width:10,height:100,score:0},s={x:e.width/2,y:e.height/2,radius:10,velocityX:0,velocityY:0,speed:a.ballSpeed},r={x:e.width/2-1,y:0,width:2,height:10};o.y=e.height/2-o.height/2,t.y=e.height/2-t.height/2,s.x=e.width/2,s.y=e.height/2,s.velocityX=0,s.velocityY=0;const i=document.getElementById("pause-btn"),l=document.getElementById("openSettings"),u=E=>{Be(E,e,o)};a.mouse&&!a.multiplayer&&e.addEventListener("mousemove",u),M(1,[{text:"Start",onClick:()=>{v=f.PLAYING,L(),_(s)}}]),i&&i.addEventListener("click",()=>{v===f.PLAYING&&(v=f.PAUSED,M(2,[{text:"Resume",onClick:()=>{v=f.PLAYING,L()}},{text:"Restart",onClick:()=>{F(e,o,t,s)}}]))}),l&&l.addEventListener("click",()=>{v===f.PLAYING&&(v=f.PAUSED,M(2,[{text:"Resume",onClick:()=>{v=f.PLAYING,L()}},{text:"Restart",onClick:()=>{F(e,o,t,s)}}]))}),window.addEventListener("keydown",E=>{E.code==="Space"&&(v===f.PLAYING?(v=f.PAUSED,M(2,[{text:"Resume",onClick:()=>{v=f.PLAYING,L()}},{text:"Restart",onClick:()=>{F(e,o,t,s)}}])):v===f.PAUSED?(v=f.PLAYING,L()):v===f.START&&(v=f.PLAYING,L(),_(s)))});function b(){if(v===f.START||v===f.PAUSED||v===f.GAME_OVER){Y(e,n,o,t,s,r);return}a.multiplayer&&Le(e,o,t),!a.multiplayer&&!a.mouse&&Se(e,o),De(e,o,t,s),Y(e,n,o,t,s,r)}function x(){(!a.mouse||a.multiplayer)&&e.removeEventListener("mousemove",u),b(),requestAnimationFrame(x)}requestAnimationFrame(x)}function Be(e,n,o){const t=n.getBoundingClientRect();let s=e.clientY-t.top-o.height/2;s<0&&(s=0),s+o.height>n.height&&(s=n.height-o.height),o.y=s}function F(e,n,o,t){n.score=0,o.score=0,n.y=e.height/2-n.height/2,o.y=e.height/2-o.height/2,t.x=e.width/2,t.y=e.height/2,t.velocityX=0,t.velocityY=0,v=f.PLAYING,L(),_(t)}function _(e){let n=e.speed;O===!1&&(n=a.ballSpeed,O=!0);const o=Math.random()*Math.PI/2-Math.PI/4,t=Math.random()<.5?1:-1;e.velocityX=t*n*Math.cos(o),e.velocityY=n*Math.sin(o)}function Le(e,n,o){A&&(n.y-=a.paddleSpeed),T&&(n.y+=a.paddleSpeed),N&&(o.y-=a.paddleSpeed),H&&(o.y+=a.paddleSpeed),n.y=Math.max(0,Math.min(n.y,e.height-n.height)),o.y=Math.max(0,Math.min(o.y,e.height-o.height))}function Se(e,n){A&&(n.y-=a.paddleSpeed),T&&(n.y+=a.paddleSpeed),n.y=Math.max(0,Math.min(n.y,e.height-n.height))}function Ie(){const e=document.getElementById("pong");return e?{canvas:e,context:e.getContext("2d")}:{canvas:null,context:null}}function D(e,n,o,t,s,r,i=!1,l="none"){e.fillStyle=r,e.fillRect(n,o,t,s),i&&(e.strokeStyle=l,e.lineWidth=2,e.strokeRect(0,0,t,s))}function W(e,n,o,t,s){e.fillStyle=s,e.beginPath(),e.arc(n,o,t,0,Math.PI*2,!1),e.closePath(),e.fill()}function z(e,n,o,t,s){e.fillStyle=s,e.font="45px fantasy",e.fillText(n.toString(),o,t)}function Me(e,n,o){W(n,o.x,0,30,a.itemsColor);for(let t=0;t<=e.height;t+=15)D(n,o.x,o.y+t,o.width,o.height,a.itemsColor)}function Y(e,n,o,t,s,r){D(n,0,0,e.width,e.height,a.bgColor,!0,a.itemsColor),Me(e,n,r),z(n,o.score,e.width/4,e.height/5,a.itemsColor),z(n,t.score,3*e.width/4,e.height/5,a.itemsColor),D(n,o.x,o.y,o.width,o.height,a.itemsColor),D(n,t.x,t.y,t.width,t.height,a.itemsColor),W(n,s.x,s.y,s.radius,a.itemsColor)}function Pe(e,n){return e.top=e.y-e.radius,e.bottom=e.y+e.radius,e.left=e.x-e.radius,e.right=e.x+e.radius,n.top=n.y,n.bottom=n.y+n.height,n.left=n.x,n.right=n.x+n.width,e.right>n.left&&e.bottom>n.top&&e.left<n.right&&e.top<n.bottom}function $(e,n){n.x=e.width/2,n.y=e.height/2,a.resetSpeed&&(n.speed=a.ballSpeed),_(n)}function Fe(e,n,o){const t=n.y+n.height/2;n.y+=(o.y-t)*a.difficulty,n.y=Math.max(0,Math.min(n.y,e.height-n.height))}function De(e,n,o,t){t.x+=t.velocityX,t.y+=t.velocityY,a.multiplayer||Fe(e,o,t),t.y+t.radius>e.height?(t.y=e.height-t.radius,t.velocityY=-t.velocityY):t.y-t.radius<0&&(t.y=t.radius,t.velocityY=-t.velocityY);const s=t.x<e.width/2?n:o;if(Pe(t,s)){j===!1&&(t.speed=a.ballSpeed,j=!0);let r=t.y-(s.y+s.height/2);r=r/(s.height/2);const i=r*Math.PI/4,l=t.x<e.width/2?1:-1;t.velocityX=l*t.speed*Math.cos(i),t.velocityY=t.speed*Math.sin(i),t.speed+=.1,l===1?t.x=s.x+s.width+t.radius:t.x=s.x-t.radius}if(t.x-t.radius<0?(o.score++,$(e,t)):t.x+t.radius>e.width&&(n.score++,$(e,t)),n.score===a.scoreLimit||o.score===a.scoreLimit){v=f.GAME_OVER;let r;n.score===a.scoreLimit?r="PLAYER 1 WINS!!!":a.multiplayer?r="PLAYER 2 WINS!!!":r="PLAYER 1 LOSES!!!",M(1,[{text:"Restart",onClick:()=>{F(e,n,o,t)}}]),Ee(r),setTimeout(()=>{Ce(n.score,o.score,a.multiplayer)},0)}}const _e=document.querySelector("#app"),U={sidebarOpen:!1};class Ae{constructor(){this._isLoggedIn=!1,this.listeners=[]}get isLoggedIn(){return this._isLoggedIn}set isLoggedIn(n){this._isLoggedIn=n,this.listeners.forEach(o=>o())}subscribe(n){this.listeners.push(n)}}const y=new Ae;async function I(e){_e.innerHTML=`
		${Q}
		...
		${ie}
		${le}
	`,V(),ge(),ye(),ke(),xe(),we()}function q(){switch(window.location.hash.slice(1)||"home"){case"about":I(ee);break;case"chat":I(te);break;case"contact":I(ne);break;case"stats":I(oe);break;case"userSettings":I(se);break;default:I(Z)}U.sidebarOpen=!1}window.addEventListener("DOMContentLoaded",q);window.addEventListener("hashchange",q);
